        ...
        if (options.coverage === 'auto') {
            ...
            env.run(function() {
                ...
                if (options.remap) {
                    ...
                    coveragePromise.resolve(remappedCoveragePromise);
                    return remappedCoveragePromise;
                }
                coveragePromise.resolve(coverage);
            });
        } else {
            coveragePromise.resolve(options.coverage);
        }
    }
};
...
let reporter = {
    ...
    report(coveragePromise, options) {
...
let uploader = {
    ...
    upload(coveragePromise, options) {
        if (options.codecov) {
...
let coverPlugin = {
    ...
    install(options) {
        ...
        coverer.cover(coveragePromise, options.cover);
        reporter.report(coveragePromise, options.report);
        uploader.upload(coveragePromise, options.upload);
    }
};
