...
(function() {
    ...
    -> that's because not was considered as a kind: 'number' instruction due to how it was implemented (extending with negated flag)
    but not will become an instruction in itself having one argument which is the negated instruction
    the consequence is that not will never conflict with other instruction because will not be registered as such
    so the problem would vanish

    BUT a better algo would allow to discover negated instruction (by adding custom logicalGenerator for Not)
    and the conflict detection must be made aware of the fact the instruction is negated or not
    but I don't quite get the implications how would you detect conflict between minimum:10 & not:minimum:5
    would you convert not:minimum:5 into a maximum:6 to facilitate conflict detection and message generation ?
    minimum instruction negation can be converted to a maximum but i'm not sure evry instruction can be converted that way
    because it's a bit complicated for now we'll just ignore negated instruction conflict
    (nothing to do instruction args are ignored by logicalGenerator on basic instruction)
    */
