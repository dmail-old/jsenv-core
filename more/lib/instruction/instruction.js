...
and we'll have logicalExpression & assignementExpression
on which we are goign to build Assertion and Formatting

not will be an instruction that will hook for complete value of an expression
and consider the boolean opposite
...
const Instruction = proto.extend('Instruction', {
    ...
    getMinArgumentLength() {
        ...
        let requiredDefinitions = signature.filter(function(argumentSignature) {
            if (argumentSignature.optional) {
                return false;
            }
            if ('default' in argumentSignature) {
                return false;
            }
            if ('autoValue' in argumentSignature) {
                return false;
            }
            return true;
        });
    ...
    setArguments(args) {
        ...
        signature.forEach(function(definition, index) {
            if (index in args === false) {
                if ('default' in definition) {
                    args[index] = definition.default;
                } else if ('autoValue' in definition) {
                    args[index] = definition.autoValue.apply(this, args);
                }
            }
        }, this);
    ...
    */
    ...
    createPathedBranch(path) {
        ...
        function prependPath(instruction) {
            let branchPath = path.slice().concat(instruction.path);
            let branchArgs = instruction.args.map(function(arg) {
                if (Instruction.isPrototypeOf(arg)) {
                    return prependPath(arg);
                }
                return arg;
            });
            let branch = instruction.extend();
...
const Input = proto.extend('Input', {
    ...
    constructor(originalValue, path = []) {
        this.path = path;
...
const Mutation = proto.extend('Mutation', {
    ...
    eval(...args) {
        return this.mutateTo('eval', ...args);
    },

    assign(...args) {
        return this.mutateTo('assign', ...args);
    },
    ...
    mutators: [
        ...
        function() {
            ...
            // run is supposed to mutate, but if it doesn't it may be due to an error
            // you can also put some logic into the afterHook when the mutation is still pure
            // after that an error will be throw if still pure
            if (this.isPure()) {
                if (hasThrown) {
                    this.instruction.failHook(this, throwedValue);
                } else {
...
Instruction.define({
    ...
    errorHook(mutation, throwedValue) {
        mutation.error(throwedValue);
        throw throwedValue;
    },

    afterHook(mutation, returnedValue) {
        // console.log('after', this.stringify(), 'output', this.output);
        // mutation.complete(returnedValue);
...
Instruction.define({
    ...
    register(name, ...args) {
        let InstructionPrototype = this.extend('Instruction ' + name, {name: name}, ...args);
...
//     i18n.addTraits({
//         not() {
//             // we should check if arguments[0] (an instruction) is instance of NotInstruction
//             // return the corresponding message
//             // not is not a trait in the new implementation it's the Not instruction which has
//             // different message depending of the instruction it's negating
//             return false;
//         }
//     });
...
// Instruction.toExpectationString = function() {
//     return this.i18n();
// };
    ...
    main(assert) {
        this.add("", function() {
            var Equals = Instruction.extend({
                constructorSignature: [{}],
                run(mutation) {
                    mutation.eval(this.args[0] === mutation.input.value);
                }
            });
