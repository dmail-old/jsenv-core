    ...
    branch(options) {
        ...
        if (options) {
            if (Options.is(options)) {
                ...
                branchOptions = Options.create(branch.options, options);
            }
        } else {
            branchOptions = Options.create(branch.options);
        }
        branch.options = branchOptions;

        if (this.hasOwnProperty('childPrototype')) {
            let childPrototype = this.childPrototype;
            let childPrototypeBranch = childPrototype.branch(Options.create(branch.options, childPrototype.options));
        ...
        if (this.hasOwnProperty('children')) {
            branch.children = this.children.map(function(child) {
                let branchChild = child.branch(Options.create(branch.options, child.options));
...
const Term = Dictionnary.registerChild('Term', parentProperties, {
    ...
    best() {
        ...
        if (match.length > 0) {
            // sort definitions by option.createDefinitionComparer
            let compare = this.options.createDefinitionComparer();
    ...
    render(scope) {
        ...
        if (!bestDefinition) {
            jsenv.warn('no definition for term', this.name);
            return this.name;
        }
...
const TermDefinition = Term.registerChild('Definition', parentProperties, {
    ...
    populate(data) {
        ...
        data.split('+').filter(function(string) {
            ...
            if (Options.has(this.options.traits, traitName) === false) {
