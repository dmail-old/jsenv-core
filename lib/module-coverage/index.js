...
const CoverageService = proto.extend('CoverageService', {
    ...
    cover(env) {
        return Promise.resolve().then(function() {
            ...
            var coverageService = this;

            this.env.run('module-coverage-report', function() {
                var donePromise;
                var coveragePromise = coverageService.collect().then(function(coverage) {
                    return coverageService.remap(coverage);
                });
                donePromise = coveragePromise;

                var reportOptions = coverageService.options.report;
                if (reportOptions) {
                    donePromise = donePromise.then(function(coverage) {
                        return coverageService.report(coverage).then(function() {
                            return coverage;
                        });
                    });
                }
                var uploadOptions = coverageService.options.upload;
                if (uploadOptions && uploadOptions.codecov) {
                    donePromise = donePromise.then(function(coverage) {
                        return coverageService.upload(coverage, uploadOptions.codecov.token).then(function() {
                            return coverage;
                        });
                    });
                }
