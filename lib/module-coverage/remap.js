...
import FileSource from 'env/file-source';
...
function remapFileCoverage(fileCoverage, sourceMap) {
    each(fileCoverage.fnMap, function(genItem, key) {
        var mapping = getMapping(sourceMap, genItem.loc);
    ...
    each(fileCoverage.statementMap).forEach(function(genItem, key) {
        var mapping = getMapping(sourceMap, genItem);
...
function remap(coverage, options) {
    ...
    var sourceCoveragePromises = Object.keys(coverage).filter(function(filePath) {
        ...
        return fileSource.import().then(function() {
            ...
            return remapFileCoverage(fileCoverage, fileSource.sourceMap.consumer);
        });
    });
