...
jsenv.build(function macro() {
    ...
    var Plugin = proto.extend.call(Action, 'Plugin', {
        ...
        build(fn) {
            if (this.phase !== 'idle') {
                throw new Error('build() must be called during idle phase');
            }
        ...
        run(data) {
            ...
            this.fn = this.createRunHook(data);
        },
        ...
        listOperations() {
            var operations = Action.listOperations.call(this);
            operations.splice(1, 0, function() {
                return this.callHook('buildHook');
            });
            operations.splice(2, 0, function() {
                return this.callHook('configHook');
            });
            operations.splice(3, 0, function(options) {
                if (options === undefined) {
                    return this.skip('no config');
                }
    ...
    var macro = {
        ...
        get(name, preventError) {
            var found = this.plugins.find(function(provider) {
        ...
        exec() {
            ...
            return pipe([
                ...
                function() {
                    return Iterable.reduceToThenable(Iterable.map(this.plugins, function(plugin) {
                        jsenv.debug('exec', plugin.name);
                        this.current = plugin;
                        return plugin.exec();
                    }, this));
                },
