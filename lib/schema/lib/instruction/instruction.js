...
not will be an instruction that will hook for success/failure of an expression
and return the opposite
doing instruction.not twice will return the original instruction untouched (not = not = noop)
and will not recursively wrap instruction into the not logic
...
const Mutation = proto.extend('Mutation', {
    ...
    fail(...args) {
        return this.mutateTo('fail', ...args);
    },

    complete(...args) {
        return this.mutateTo('complete', ...args);
    },
...
Instruction.define({
    ...
    failHook(mutation, throwedValue) {
        mutation.fail(throwedValue);
        throw throwedValue;
    },
