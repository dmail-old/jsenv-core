...
const Instruction = proto.extend('Instruction', {
    ...
    */
    ...
    throwHook(throwedValue) {
        this.throw(throwedValue);
        throw throwedValue;
    },

    afterHook(returnedValue) {
        // console.log('after', this.stringify(), 'output', this.output);
        this.return(returnedValue);
    },
...
const MutableState = proto.extend('MutableState', {
    defaultState: 'default',
    defaultMutationValue: undefined,
    defaultMutationOrigin: undefined,
    mutationName: undefined,
    mutationValue: undefined,
    mutationOrigin: undefined,

    constructor(defaultState, defaultMutationValue, defaultMutationOrigin) {
        this.defaultState = defaultState;
        this.defaultMutationValue = defaultMutationValue;
        this.defaultMutationOrigin = defaultMutationOrigin;
    ...
    isMutated() {
        return this.isPure() === false;
    },

    mutate(mutationName, mutationValue, mutationOrigin) {
        if (this.isMutated()) {
            throw new Error(
                'MutableState.mutate("' + mutationName + '") must not be called once state is mutated'
            );
        }

        let length = arguments.length;

        this.state = mutationName;

        if (length > 1) {
            this.mutationValue = mutationValue;
        } else {
            this.mutationValue = this.defaultMutationValue;
        }

        if (length > 2) {
            this.mutationOrigin = mutationOrigin;
        } else {
            this.mutationOrigin = this.defaultMutationOrigin;
        }

        var mutation = {
            name: mutationName,
            value: this.mutationValue,
            origin: this.mutationOrigin
        };

        this.mutationHook(mutation);
    }
});

const InstructionInput = proto.extend('InstructionInput', {
    container: null,
    has: false,
    owner: null,
...
const InstructionOutput = proto.extend('InstructionOutput', {
    has: false,
    value: undefined,
...
const CompiledInstructionProperties = {
    compileOptions: null,
    state: undefined,
    input: undefined,
    output: undefined,

    compileConstructor(value, options = {}) {
        this.input = InstructionInput.create(value, this.path);
        this.output = InstructionOutput.create();

        this.state = MutableState.create('default', undefined, this); // 'default', 'skipped', 'errored', 'returned'
        this.state.mutationHook = function(mutation) {
            this.output.value = mutation.value;
        }.bind(this);
    ...
    skip(...args) {
        return this.state.mutate('skipped', ...args);
    },

    throw(...args) {
        return this.state.mutate('errored', ...args);
    },

    return(...args) {
        return this.state.mutate('returned', ...args);
    },

    // call this when instruction want to transform the input value
    transform(transformedInputValue) {
        if (this.state.isMutated()) {
        ...
        var stateMutators = [
            function() {
                if (this.input.has === false) {
                    this.valueNotFoundHook();
                }
            },
            function() {
                this.beforeHook(this.input);
            },
            function() {
                let returnValue;
                let hasThrown = false;
                ...
                try {
                    returnValue = this.run(this.input);
                } catch (e) {
                    hasThrown = true;
                    throwedValue = e;
                }
                ...
                if (hasThrown) {
                    this.throwHook(throwedValue);
                } else {
                    this.afterHook(returnValue);
                }
            },
            function() {
                throw new Error('instruction.exec() must lead to a state mutation');
            }
        ];

        if (this.state.isPure()) {
            for (let stateMutator of stateMutators) {
                stateMutator.call(this);
                if (this.state.isMutated()) {
                    break;
                }
            }
...
Instruction.define({
    compiledInstructionProperties: CompiledInstructionProperties,
    ...
    compile(...args) {
        ...
        compiledInstruction.define(this.compiledInstructionProperties);
        compiledInstruction.compileConstructor(...args);

        return compiledInstruction;
    },

    eval(...args) {
        return this.compile(...args).exec().output.value;
    }
});
    ...
    main(assert) {
        this.add("", function() {
            var Equals = Instruction.extend({
                constructorSignature: [{}],
                run(input) {
                    return this.args[0] === input.value;
                }
            });
