...
const CompiledInstructionProperties = {
    ...
    state: 'default', // 'default', 'skipped', 'errored', 'returned'
    output: undefined,
    ...
    setState(stateName, output) {
        if (this.state !== 'default') {
            throw new Error(
                'instruction.setState("' + stateName + '") must be called while instruction state is default'
            );
        }

        // console.log(stateName, this.stringify(), output);

        this.state = stateName;
        this.output = output;
    },

    // call this when instruction mus tbe skipped for a reason
    skip(skipReason) {
        return this.setState('skip', skipReason);
    },

    throw(throwedValue) {
        return this.setState('threw', throwedValue);
    },

    // call this to force instruction as having returned with value passed in argument
    return(returnedValue) {
        return this.setState('returned', returnedValue);
    },

    // call this when instruction want to transform the input value
    transform(transformedInputValue) {
        if (this.state !== 'default') {
            throw new Error('instruction.transform() must be called while instruction state is default');
        }
        ...
        if (owner !== undefined) {
            ...
            owner[propertyName] = transformedInputValue;
        }
        ...
        var stateMutators = [
            ...
            function() {
                ...
                if (hasThrown) {
                    ...
                    this.after(returnValue);
                }
            },
            function() {
                throw new Error('instruction state must not be default after being evaluated');
            }
        ];
...
Instruction.define({
    ...
    eval(...args) {
        return this.compile(...args).exec().output;
    }
});
