...
// remember also there is two things : foramtting a value & validating a value
const Mutation = proto.extend('Mutation', {
    ...
    mutate(mutationName, mutationValue, mutationOrigin) {
        if (this.isMutated()) {
            throw new Error(
                'Mutation.mutate("' + mutationName + '") must not be called once state is mutated'
            );
        }
...
const CompiledInstructionProperties = {
    ...
    compileConstructor(value, options = {}) {
        ...
        this.mutation = Mutation.create(undefined, this); // 'default', 'skipped', 'errored', 'returned'
        this.mutation.mutatedHook = function(mutation) {
    ...
    skip(...args) {
        return this.mutation.mutate('skipped', ...args);
    },

    throw(...args) {
        return this.mutation.mutate('errored', ...args);
    },

    return(...args) {
        return this.mutation.mutate('returned', ...args);
    },

    transform(transformedInputValue) {
        if (this.mutation.isMutated()) {
    ...
    main(assert) {
        this.add("", function() {
            var Equals = Instruction.extend({
                constructorSignature: [{}],
                run(compiledInstruction) {
                    return this.args[0] === compiledInstruction.input.value;
                }
            });
