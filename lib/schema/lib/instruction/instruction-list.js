...
const InstructionList = Instruction.extend('InstructionList', {
    ...
    add(...args) {
        return this.args.push(this.createCombinedInstruction(...args));
    },
        ...
        if (previousMutationChild) {
            ...
            if (operator === 'and') {
                if (previousMutationChild.output.isFalsy()) {
                    ...
                    //     'is falsy'
                    // );
                    mutationChild.complete(false, previousMutationChild);
                }
            } else if (operator === 'or') {
                if (previousMutationChild.output.isTruthy()) {
                    mutationChild.complete(true, previousMutationChild);
                }
            }
        }
    },
    ...
    afterHook(mutation) {
        ...
        if (mutationLastChild) {
            mutation.complete(mutationLastChild.output.isTruthy(), mutationLastChild);
        } else {
...
const OneOf = registerInstructionList('oneOf', {
    ...
    afterEachHook(mutation, mutationChild) {
        ...
        if (mutationChild.output.isTruthy()) {
            ...
            if (mutation.meta.truthyCount > 1) {
                mutation.complete(false, mutationChild);
            }
            // console.log(
            //     mutationChild.instruction.stringify(),
    ...
    afterHook(mutation) {
        ...
        mutation.complete(exactlyOneInstructionIsTruthy);
    }
});
...
export const test = {
    ...
    main(assert) {
        var equals = Instruction.extend('equals', {
            constructorSignature: [{}],
            run(mutation) {
                return mutation.input.value === this.args[0];
            }
        });
