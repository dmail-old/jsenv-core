...
var StackTrace = {
    constructor: function(error, v8CallSites) {
        ...
        if (is(error)) {
            if (v8CallSites) {
                ...
                this.expectedText = error.expectedText;
                this.actual = error.actual;
                this.actualText = error.actualText;
                this.hint = error.hint;
                this.hintSource = error.hintSource;
    ...
    get stack() {
        ...
        stack += '\n-- ' + this.name.toUpperCase() + ' --------------------------------------'; // the amount of - could depend on terminal width
        stack += '\n\n';
        ...
        if (this.callSites.length) {
            stack += this.callSites[0].toCallString();
        }
        if (this.expectedText) {
            stack += this.expectedText + '\n\n';
        }
        if (this.expected) {
            stack += this.expected + '\n\n';
        }
        if (this.actualText) {
            stack += this.actualText + '\n\n';
        }
        if (this.actual) {
            stack += this.actual + '\n\n';
        }
        if (this.hint) {
            stack += 'Hint: ' + this.hint + '\n\n';
            if (this.hintSource) {
                stack += '\t' + this.hintSource + '\n\n';
            }
        }
