...
var rest = proto.extend('Rest', {
    ...
    createRequest(uri, options) {
        return this.Request.create(uri, options);
    },
    ...
    fetch(firstArg, options) {
        var request;
        var responsePromise;

        if (this.Request.isPrototypeOf(firstArg)) {
            request = firstArg.clone(options);
    ...
    fetchCustom(uri, options, forceOptions) {
        var requestOptions;
        if (options) {
    ...
    head(uri, options = {}) {
        return this.fetchCustom(uri, options, {
            method: 'HEAD'
        });
    },
