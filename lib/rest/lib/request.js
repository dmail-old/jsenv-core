    ...
    Object.keys(defaultOptions).forEach(function(optionName) {
        ...
        if (firstHasValue && secondHasValue) {
            if (optionName === 'headers') {
                optionValue = Item.concat(firstOptions[optionName], secondOptions[optionName]);
            } else {
...
const Request = proto.extend('Request', BodyConsumer, {
    uri: env.baseURI,
    ...
    equals(otherRequest) {
        if (this.uri.equals(otherRequest.uri) === false) {
            return false;
        }
        // we haven't yet implemented the Headers.prototype.equals
        if (this.headers.equals(otherRequest.headers) === false) {
            return false;
        }
        return true;
        // for the body it's more complicated we can't know if they are the same
    },
    ...
    clone(uri, options) {
        ...
        var cloneRequest = this.create(uri, options);

        return cloneRequest;
    },

    redirect(uri) {
        return this.clone(uri);
    }
});
