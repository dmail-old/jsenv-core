...
function transformCallSite(callSite, index, callSites) {
    callSite.ready = function() {
        var sourceLocation = this.getScriptNameOrSourceURL() || this.getFileName();

        if (sourceLocation) {
            var line = this.getLineNumber();
            var column = this.getColumnNumber() - 1;
            ...
            // instead of doing this sync, we will do this async trying to recursively find the rootSource position
            // by fetching sourceMap
            ...
            this.columnNumber = position.column + 1;
        }
    };
