...
const Variable = templateExports.Variable;
const ReferencableVariable = Variable.extend({
    ...
    toString() {
        var string = Variable.toString.call(this);
        if (this.isReference) {
...
ReferencableVariable.compiler = Variable.compiler.extend({
    compile(syntaxNode) {
        let compiledSyntaxNode = Variable.compiler.compile(syntaxNode);
...
// trait should be compilable too according to the fact you can negate them
// they look like transformer but they returns Boolean indicating if something match or not
// a variable may have n transformers and a definition may have n traits
// they share the same API and the same format you can register trait or transformer and parametrize the the same way
const Trait = proto.extend('Trait', {

});
const Template = templateExports.Template;
const Definition = Template.extend({
    name: 'definition',
    childrenMap: {
        variable: ReferencableVariable,
        constant: templateExports.Constant
    },

    traitPrototypes: [Trait],
...
export {ReferencableVariable, Trait, Definition};
