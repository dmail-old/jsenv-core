...
var ValueDefinition = (function() {
    ...
    var ValueDefinition = createConstructor({
        ...
        concat(valueDefinition) {
            ...
            if (this.isPrimitive || valueDefinition.isPrimitive) {
...
// var toStringTagSymbol = typeof Symbol === 'undefined' ? '@@toStringTag' : Symbol.toStringTag;
...
var ValueGenerator = createConstructor({
    ...
    registerGenerator(object, generateMethod) {
        var generator = {
            object: object,
            string: prototypeToString.call(object),
            create: generateMethod
        };
        this.generators.push(generator);
        return generator;
    },

    findGeneratorByValuePrototype(value) {
        ...
        if (speciesSymbol in value) {
            // for different frame speciesSymbol will not be readable for the value
            // that's why the second way to get a valuePrototype is to compare
            // registered prototype to their toStringTag values (check via Object.prototype.toString for compat)
            // and that's why all prototype must be registered on ValueGenerator
            // in short doing proto.extend() must register the created object into ValueGenerator
            var species = value[speciesSymbol];

            for (; i < j; i++) {
                generator = generators[i];
                if (generator.object.constructor === species) {
            ...
            for (; i < j; i++) {
                generator = generators[i];
                if (generator.object.isPrototypeOf(value)) {
                    foundGenerator = generator;
                    break;
                }
            }
    ...
    findGeneratorByValuePrototypeString(value) {
        ...
        // both are weaknesses of this function an inherent to javaScript but
        // for now it should not have negative impact
        // the below implementation suggest we could simplify into one loop but keep this optimization for later
        // when we will know if the above weaknesses are important or not
        // we must be aware that
        var generators = this.generators;
        var i = 0;
        var j = generators.length;
        var foundGenerator;
        var valuePrototypeString = prototypeToString.call(value);

        for (;i < j; i++) {
            var generator = generators[i];
            if (generator.string === valuePrototypeString) {
                foundGenerator = generator;
                break;
            }
        }
    ...
    findGeneratorByValue(value) {
        var generatorFound;
        var generatorFoundByValuePrototype = this.findGeneratorByValuePrototype(value);
        if (generatorFoundByValuePrototype) {
            generatorFound = generatorFoundByValuePrototype;
        } else {
            // if nothing has matched it may be due to the fact value comes from outside of this script
            // as a consequence value instanceof Object is false
            // but we can still know the value type using Object.prototype.toString strategy
            // because naming during proto.extend is optional we keep the isPrototypeOf() check
            var generatorFoundByValuePrototypeString = this.findGeneratorByValuePrototypeString(value);
            generatorFound = generatorFoundByValuePrototypeString;
        }
    ...
    createValue() {
        ...
        if (valueDefinition.isPrimitive) {
            ...
            var generator = this.findGeneratorByValue(value);

            if (generator) {
                createdValue = generator.create(value);
            } else {
                // the only way I see to reach this would be to write
                // Object.prototype[Symbol.toStringTag] = 'foo';
                throw new Error('never suppoed to happen, the value is not even an object');
            }
        }
...
function registerGenerator() {
    return ValueGenerator.prototype.registerGenerator.apply(ValueGenerator.prototype, arguments);
}

registerGenerator(Array.prototype, function(value) {
    return new Array(value.length);
});
registerGenerator(Date.prototype, function(value) {
...
registerGenerator(Function.prototype, function(value) {
    return value;
});
registerGenerator(RegExp.prototype, function(value) {
    return new RegExp(value.valueOf());
});
// keep object last because all is instanceof Object
registerGenerator(Object.prototype, function(value) {
    if (speciesSymbol in value) {
        return Object.create(value[speciesSymbol].prototype);
    }
