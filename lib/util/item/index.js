    ...
    var ValueDefinition = createConstructor({
        ...
        clonePropertyDefinition(propertyDefinition) {
            ...
            if (propertyValueDefinition) {
                var clonedPropertyValueDefinition = new propertyValueDefinition.constructor();

                clonedPropertyValueDefinition.parent = this;

                var reference = clonedPropertyValueDefinition.findReference(clonedPropertyValueDefinition.value);
                if (reference) {
                    // console.log('reference found for', this.value,
                    //     'of property', propertyDefinition.name, 'at', reference);
                    clonedPropertyValueDefinition.markAsReferenceTo(reference);
                } else {
                    clonedPropertyValueDefinition.adopt(propertyValueDefinition);
                    var clonedPropertyDefinitions = this.propertyDefinitions.map(
                        clonedPropertyValueDefinition.clonePropertyDefinition,
                        clonedPropertyValueDefinition
                    );
                    clonedPropertyValueDefinition.propertyDefinitions = clonedPropertyDefinitions;
                }
        ...
        concat(valueDefinition) {
            ...
            if (this.primitiveMark || valueDefinition.primitiveMark) {
                // concatenedDefinition.parent = valueDefinition.parent;
                concatenedDefinition.adopt(valueDefinition);
            } else {
                // concatenedDefinition.parent = this.parent;
                concatenedDefinition.adopt(this);
