...
var ValueGenerator = (function() {
    ...
    var PropertyDefinition = createConstructor({
        constructor(name, owner) {
            ...
            this.referenceOf = null; // for later, a propertyDefinition can be a reference to an other one
            // we will detect that keeping propertyDefinition array and checking the descriptor.value
            // if they match === so the propertyDefinition is a referenceto an other
            // in some circumstances we will speed up things when we know a property definition is a reference to an other
        },
        ...
        var ValueGenerator = createConstructor({
            ...
            produce() {
                ...
                if (isPrimitive(value)) {
                    ...
                    var references;
                    if (this.parent) {
                        references = this.parent.references;
                    ...
                    if (reference) {
                        ...
                        if (argsLength > 0 && isPrimitive(args[argsLength - 1])) {
                            ...
                            var mergedArgumentDefinitions = this.mergeDefinitions(argumentDefinitions);
                            this.propertyDefinitions.push(...mergedArgumentDefinitions);
                        }
                    }
                }
