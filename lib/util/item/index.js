...
var ValueDefinition = (function() {
    ...
    var ValueDefinition = createConstructor({
        ...
        populate(value) {
            ...
            if (isPrimitive(value) === false) {
                ...
                propertyNames.forEach(function(propertyName) {
                    var propertyDefinition = new this.constructor(this, propertyName);
        ...
        concat(valueDefinition) {
            var concatenedDefinition = new valueDefinition.constructor();
            // we don't care about descriptor for now
            // var descriptor = {};
            // if (this.descriptor) {
            //     Object.assign(descriptor, this.descriptor);
            // }
            // if (valueDefinition.descriptor) {
            //     Object.assign(descriptor, valueDefinition.descriptor);
            // }
            // concatenedDefinition.descriptor = descriptor;

            concatenedDefinition.name = valueDefinition.name;

            if (valueDefinition.isPrimitive()) {
                concatenedDefinition.value = valueDefinition.value;
            } else {
                // the concatenedDefinition value is unkown, we don't care about it, it will be dynamically set
                var propertyDefinitions = concatenedDefinition.propertyDefinitions;

                propertyDefinitions.push(...this.propertyDefinitions);
                valueDefinition.propertyDefinitions.forEach(function(propertyDefinition) {
                    var propertyDefinitionIndex = propertyDefinitions.findIndex(function(concatenedPropertyDefinition) {
                        return concatenedPropertyDefinition.name === propertyDefinition.name;
                    });

                    if (propertyDefinitionIndex === -1) {
                        propertyDefinitions.push(propertyDefinition);
                    } else {
                        var existingPropertyDefinition = propertyDefinitions[propertyDefinitionIndex];
                        var concatenedPropertyDefinition = existingPropertyDefinition.concat(propertyDefinition);

                        propertyDefinitions.splice(propertyDefinitionIndex, 1, concatenedPropertyDefinition);
                    }
                }, this);
            }
...
function createValueDefinition(value) {
    ...
    valueDefinition.populate(value);
    return valueDefinition;
}
...
export const test = {
    ...
    main(assert) {
        this.add('core', function() {
            var definition = createValueDefinition({
                foo: {
                    bar: true
                }
            });
            var fooDefinition = definition.propertyDefinitions[0];
            var barDefinition = fooDefinition.propertyDefinitions[0];

            assert(fooDefinition.name === 'foo');
            assert(fooDefinition.owner === definition);
            assert(barDefinition.name === 'bar');
            assert(barDefinition.owner === fooDefinition);
            assert(barDefinition.value === true);
        });
        ...
        this.add('concat', function() {
            ...
            assert(firstNameDefinition.value === 'ok');
            assert(thirdNameDefinition.value === 'boo');
            assert(thirdAgeDefinition.value === 10);
        });
    }
};
