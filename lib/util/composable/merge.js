...
FunctionObjectElement.refine({
    conflictResolutionStrategy: 'replace', // but we may allow 'cbomine' with the reduce method below
...
// every property which are array index would be incremented resulting in the concatenation of the array
// I suppose it means conflictResolutionStrategy string would be dynamic
// and when the property is an array index the strategy could change or sthing like that
// or we could define a new resolution mecasnim that would clone the property with a different name
// this behaviour could be disabled I don't know how
// it also show that array could be combined with object & object with array
// in the same fashion Array & Set could be combined, for now we ignore thoose exotic case
