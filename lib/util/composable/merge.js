...
Element.refine({
    compose(secondElement) {
        const composedElement = this.collide(secondElement);
        composedElement.propagateReaction();
    ...
    reactWith(secondElement) {
        ...
        if (conflictResolutionStrategy === 'replace') {
            reactionMethod = 'cloneReaction';
            reactionArgs = [secondElement];
        } else if (conflictResolutionStrategy === 'reverseReplace') {
            reactionMethod = 'cloneReaction';
            reactionArgs = [firstElement];
        } else if (conflictResolutionStrategy === 'combine') {
            reactionMethod = 'fusionReaction';
            reactionArgs = [firstElement, secondElement];
        } else if (conflictResolutionStrategy === 'reverseCombine') {
            reactionMethod = 'fusionReaction';
            reactionArgs = [firstElement, secondElement];
        }

        // console.log('reaction between', firstElement.value, 'and', secondElement.value, 'is', reactionMethod);

        reaction.catalyst = firstElement;
        reaction.method = reactionMethod;
        reaction.args = reactionArgs;
        reaction.react = function() {
            return this.catalyst[this.method](...this.args);
        };
    ...
    cloneReaction(element) {
        const clone = element.clone(false);

        clone.propagateReaction = function() {
            clone.cloneChildren(element, true);
        };
    ...
    fusionReaction(firstElement, secondElement) {
        // the collided particles (elements) are transformed into one merged element
        const catalystElement = this;
        const firstElementValue = firstElement.value;
        const secondElementValue = secondElement.value;
        const mergedValue = catalystElement.merge(firstElementValue, secondElementValue);
...
ObjectElement.refine({
    ...
    propagateReaction() {
        ...
        // mais on s'en fous
        this.importProperties(this.firstComponent);
        this.importProperties(this.secondComponent);
    },
...
ArrayElement.refine({
    conflictResolutionStrategy: 'combine',
    propagateReaction() {
        ObjectElement.propagateReaction.call(this);
    },
    merge() {
...
ArrayPropertyElement.refine({
    ...
    fusionReaction(firstArrayPropertyElement, secondArrayPropertyElement) {
        // je pense que ce sera plutot dans propagateReaction ou pas parce que l'idée
        // c'est d'éviter la fusion de deux propriétés pour lesquelles isIndex() vaut true
        // dans propagate reaction c'est déjà "trop tard"
        // c'est donc plutot dans le reactWith qu'il faut agir

        const isArrayConcatenationEnabledBySomething = true;

        if (isArrayConcatenationEnabledBySomething) {
            if (firstArrayPropertyElement.name === 'length') {
            ...
            } else if (firstArrayPropertyElement.isIndex()) {
                const firstArray = firstArrayPropertyElement.parentNode;
                ...
                const firstIndexedProperty = firstArrayPropertyElement.clone(false);
                const secondIndexedProperty = secondArrayPropertyElement.clone(false);
                const secondIndexedPropertyCurrentIndex = Number(secondArrayPropertyElement.name);
                ...
                return [
                    propertyFragment,
                    function() {
                        firstIndexedProperty.cloneChildren(firstArrayPropertyElement, true);
                        secondIndexedProperty.cloneChildren(secondArrayPropertyElement, true);
                    }
                ];
            }
        }

        return ObjectPropertyElement.lazyCombine.apply(this, arguments);
    }
});
...
StringElement.refine({
    conflictResolutionStrategy: 'combine', // unlike primitive there is property to combine but secondString is kept (sort of replace)
