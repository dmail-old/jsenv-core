...
Element.refine({
    compose(secondElement) {
        ...
        if (conflictResolutionStrategy === 'ignore') {
            const clonedFirstElement = firstElement.clone(false);
            clonedFirstElement.cloneChildren(firstElement);
            elementComposed = clonedFirstElement;
        } else if (conflictResolutionStrategy === 'replace') {
            const clonedSecondElement = secondElement.clone(false);
            clonedSecondElement.cloneChildren(secondElement);
            ...
            combinedElement.importChildren(firstElement);
            combinedElement.importChildren(secondElement);
    ...
    cloneChildren(element) {
        for (let child of element) {
            this.appendChild(child.clone(false));
        }
    },
    ...
    importChild(element) {
        ...
        if (currentChild) {
            ...
            if (conflictResolutionStrategy === 'ignore') {
                ...
                const elementCombined = this.combine(currentChild, elementToImport);
                parentElement.replaceChild(currentChild, elementCombined);
                elementCombined.importChildren(currentChild);
                elementCombined.importChildren(elementCombined);
