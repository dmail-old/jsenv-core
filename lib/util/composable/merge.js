...
Element.refine({
    compose(element) {
        ...
        if (combine === 'replace') {
            // l'élément composé est un clone de l'élément qu'on compose et on retourne
            // le dit élément
            // compose({foo: true}).compose({foo: false}) va donc retourner
            // un arbre fooElement dans lequel le trueElement est remplacé par falseElement
            // l'arbre a donc mutate ce qu'on veut éviter
            // si par contre on retourne une version cloner de l'élément c'est bon
            // en revanche ce composedElement doit être insérer dans le cas d'un nested compose()
            composedElement = element.clone();
        } else if (combine === 'ignore') {
            composedElement = this.clone();
        } else {
            composedElement = combine.call(this, this, element);
        }
    ...
    combine(firstElement, secondElement) {
        const combinedElement = firstElement.clone(false);
        const data = firstElement.data;
        const dataToCombine = secondElement.data;
        const combinedData = this.combineData(data, dataToCombine);

        combinedElement.firstComponent = firstElement;
        combinedElement.secondComponent = secondElement;
        combinedElement.data = combinedData;

        /*
        ça ne peut pas être si simple
        déjà il faut copier les child
        ensuite faut que si deux childs sont en conflit on les merge
        qu'est ce qui définit que dexu child sont en conflits ?
        ça a priori ça appartient à l'élément parce que
        object dira que c'est propertyName
        array que c'est propertyIndex (et on pourrait choisir de concat pour éviter le conflit)

        ok y'a un gros grap niveau comment on ve faire ça, ça a toujours été le truc relou pas bien clair
        */
        for (let child of firstElement) {
            combinedElement.appendChild(child.clone());
        }
        for (let child of secondElement) {
            combinedElement.appendChild(child.clone());
        }
...
NumberElement.refine({
    combineData(firstNumber, secondNumber) {
        return firstNumber + secondNumber;
    }
});
StringElement.refine({
    combineData(firstString, secondString) {
...
ObjectPropertyElement.refine({
    combine(firstObjectPropertyElement, secondObjectPropertyElement) {
        ...
        const combinedObjectPropertyElement = firstObjectPropertyElement.clone(false);

        combinedObjectPropertyElement.appendChild(
            firstObjectPropertyElement.children[0].compose(
                secondObjectPropertyElement.children[0]
            )
        );

        return combinedObjectPropertyElement;
...
FunctionObjectElement.refine({
    combineData(firstFunction, secondFunction) {
        return function() {
