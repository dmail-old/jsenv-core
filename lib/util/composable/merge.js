...
const CombinePropertyReaction = CombineObjectReaction.extend({
    refine(combinedProperty, firstProperty, secondProperty) {
        ...
        if (situation === 'value-value') {
            ...
            const propertyReaction = firstPropertyValueElement.reactWith(secondPropertyValueElement);
            combinedProperty.appendChild(propertyReaction.prepare());
...
const DynamicIndexConcatenationReaction = Reaction.extend({
    produce(firstArrayProperty, secondArrayProperty) {
        ...
        const concatenedProperty = propertyTransformation.produce();
...
const CombineArrayPropetyReaction = createDynamicReaction(
    function(element) {
...
ArrayPropertyElement.refine({
    reaction: CombineArrayPropetyReaction,
    isIndex() {
