...
Element.refine({
    ...
    transform() {
        const transformation = this.transformation.create(this);
...
const CopyObjectPropertyTransformation = CopyTransformation.extend({
    // y'a un truc à faire pour que descriptor soit préservé
    // soit descriptor devient value (j'aime ça)
    // soit descriptor doit aussi dire c'est quoi le nom de la propriété de sorte
    // qu'on a toute la logique qui se trouve dans property.value qui contient le descriptor
});
...
const CombineObjectReaction = Reaction.extend({
    ...
    handleNewProperty(element, property) {
        const transformation = property.transform();
    ...
    handleComponent(combinedProperty, componentName, firstProperty, secondProperty) {
        ...
        if (firstComponent) {
            ...
            if (firstComponent && secondComponent) {
                reaction = firstComponent.reactWith(secondComponent);
            } else if (firstComponent) {
                reaction = firstComponent.transform();
            }
...
const CombineArrayReaction = CombineObjectReaction.extend({
    ...
    handlePropertyCollision(mergedArray, currentProperty, conflictualProperty) {
        if (currentProperty.isIndex()) {
            ...
            const propertyTransformation = conflictualProperty.transform();
...
ArrayElement.refine({
    ...
    transformaion: CloneArrayTransformation
});
