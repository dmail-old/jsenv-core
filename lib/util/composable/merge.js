...
const CombineObjectReaction = Reaction.extend({
    ...
    refine(mergedObject, firstElement, secondElement) {
        this.importProperties(mergedObject, firstElement);
        this.importProperties(mergedObject, secondElement);
    },
    ...
    importProperty(element, property) {
        ...
        if (currentProperty) {
            importedProperty = this.handlePropertyCollision(element, currentProperty, propertyToImport);
        } else {
            importedProperty = this.handleNewProperty(element, propertyToImport);
        }

        return importedProperty;
    },

    handlePropertyCollision(element, currentProperty, property) {
        ...
        const reaction = currentProperty.reactWith(property);
        const importedProperty = reaction.prepare();
        this.replaceProperty(element, currentProperty, importedProperty);
        reaction.proceed();
        return importedProperty;
    },

    replaceProperty(element, currentProperty, property) {
        element.replaceChild(currentProperty, property);
    },

    handleNewProperty(element, property) {
        const transformation = CloneTransformation.create(property);
...
const CombinePropertyReaction = CombineObjectReaction.extend({
    refine(combinedProperty, firstProperty, secondProperty) {
        ...
        combinedProperty.descriptor = combinedDescriptor;
    ...
    handleComponent(combinedProperty, componentName, firstProperty, secondProperty) {
        ...
        if (firstComponent) {
            ...
            if (firstComponent && secondComponent) {
                reaction = this.handleComponentCollision(firstComponent, secondComponent);
            } else if (firstComponent) {
                reaction = this.handleNewComponent(firstComponent);
            }
    ...
    handleComponentCollision(combinedProperty, firstComponent, secondComponent) {
        const propertyReaction = firstComponent.reactWith(secondComponent);
        return propertyReaction;
    },

    handleNewComponent(combinedProperty, component) {
        const cloneTransformation = CloneTransformation.create(component);
        return cloneTransformation;
    },
...
const CombineArrayReaction = CombineObjectReaction.extend({
    ...
    refine(mergedArray) {
        const combinedLengthProperty = mergedArray.createProperty('length');
        mergedArray.addProperty(combinedLengthProperty);
        combinedLengthProperty.fill(0);
    ...
    handlePropertyCollision(mergedArray, currentProperty, conflictualProperty) {
        if (currentProperty.isIndex()) {
            const mergedArrayLengthProperty = mergedArray.getProperty('length');
            const mergedArrayLength = mergedArrayLengthProperty.propertyValue;
            const conflictualIndex = Number(currentProperty.name);
            const concatenedIndex = conflictualIndex + mergedArrayLength;
            const concatenedIndexAsString = String(concatenedIndex);
            // the fact we clone the conflictualProperty could be an option as well
            const propertyTransformation = CloneTransformation.create(conflictualProperty);
            const concatenedProperty = propertyTransformation.prepare();

            concatenedProperty.name = concatenedIndexAsString;
            this.addProperty(mergedArray, concatenedProperty);
            propertyTransformation.proceed();
            return concatenedProperty;
        }
        return CombineObjectReaction.handlePropertyCollision.apply(this, arguments);
    },

    addProperty(mergedArray, property) {
        if (property.isIndex()) {
            mergedArray.getProperty('length').valueNode.value++;
        }
        return CombineObjectReaction.addProperty.apply(this, arguments);
    }
});
