...
Element.refine({
    compose(secondElement) {
        ...
        if (conflictResolutionStrategy === 'ignore') {
            ...
            const combinedElement = secondElement.combine(firstElement);
            combinedElement.initialize();
            elementComposed = combinedElement;
        }
    ...
    importChild(element) {
        ...
        if (currentChild) {
            ...
            if (conflictResolutionStrategy === 'ignore') {
                ...
                const elementImported = elementToImport.clone(false);
                ...
                const elementCombined = elementToImport.combine(currentChild);
                parentElement.replaceChild(currentChild, elementCombined);
                elementCombined.initialize();
                child = elementCombined;
            }
...
FunctionObjectElement.refine({
    conflictResolutionStrategy: 'replace', // but we may allow specific reduce method as below
