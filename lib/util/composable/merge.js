...
Element.refine({
    merge(mergedElement) {
        ...
        const dataToMerge = mergedElement.data;
        const mergedData = element.combine(data, dataToMerge);

        element.data = mergedData;

        /*
        to keep the original data we may replace this element by a fragment holding both like
        so function may know how they are composed, we dont lost how the tree was composed
        as a consequence
        - tree could be serialized
        - what happens when you do rootElement.merge ? it cannot be replaced so you would return the fragment element in that case
        but thanks to the tree structure calling rootElement.merge remains ok because event if it's not the rootElement anymore
        the tree structure remains valid
        however you need the rootElement in order to serialize from the root

        const fragment = element.createConstructor();
        this.replace(fragment);
        fragment.appendChild(this);
        fragment.appendChild(mergedElement);
        fragment.data = mergedData;
        return fragment;

        imagine you merge two functionElement
        the merged element must create a wrapped function callin both
        and then add all the eventual function properties on the newly created function

        so you have the fucntion fragment which says:
        - I'm using a wrapper function calling both function
        - My first child is first function
        - My second child is second function
        - what are my properties ? how are they represented

        maybe we could see function fragment as being the combination of two function
        having its children set a the merged list of properties
        and store firstFunctionElement & secondFunctionElement as attributes not children

        some would apply for objects
        they would be an objectFragment (or composedObject) which says I'm the composition of two objects
        here are my properties & .data would be the resulting object
        and composedObject could keep reference two it's component using attributes

        we must introduce the attributes concept (value which are keps during cloning etc but are not part of the tree)
        we may rename merge into combine or sthing else to prevent confusion
        combine will sometime mutate the element sometime not (or every time it mutates but may returns something different)
        in that case it may remain merge

        we could also just modify this element by adding it's composition into a given array like

        // keep track of composition
        this.composition.push(this.data);
        this.composition.push(mergedElement.data);
        // combine data
        this.data = this.combine(this.data, mergedElement.data);
        // combine children
        for (child of mergedElement) {
    ...
    // merge calls combine
    // an option could allow merge two reverse combine argument so that it would call combine(mergedData, data);
    // on other option to allow merge to replace on eof the two argument in other words one element would takes precedence
    // in that scenario combine is useless
    // and a way to set you orwn combine method to allow substract divide the data instead of the default + operator
    // combine MUST BE pure : it must not mutate its argument (see ObjectElement.combine for instance)
    // im not sure it must be pure we would loose the benefits of merge which clones the element
    // however we cannot clone element.data without lab.js
    combine(firstData, secondData) {
...
ObjectElement.refine({
    combine(firstObject, secondObject) {
        // ideally more like Object.defineDescriptors(Object.getOwnPropertyDescriptors())
        // anyway this object is not meant to be used for now so it does not have to be relevant
        return Object.assign({}, firstObject, secondObject);
    },

    merge(mergedObjectElement) {
        Element.merge.call(this, mergedObjectElement);

        // when we merge we add mergedElement children at the end
        // on ther merge strategy could put thoose merged children at the begining
        for (let property of mergedObjectElement) {
...
ObjectPropertyElement.refine({
    merge(propery) {
        ...
        this.children[0].merge(propery.children[0]);
        return this;
...
FunctionObjectElement.refine({
    combine(firstFunction, secondFunction) {
        return function() {
...
ArrayObjectElement.refine({
    // i'm not sure array need a specific combine method because just like object
    // the element.data does not matter it's just a container of properties
