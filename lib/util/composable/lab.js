...
const ObjectElement = Element.extend('Object', {
    ...
    fill(value) {
        ...
        Object.getOwnPropertyNames(value).filter(this.filterProperty, this).forEach(function(name) {
            const propertyNode = this.createProperty(name);
            const descriptor = Object.getOwnPropertyDescriptor(value, name);
            this.addProperty(propertyNode);
            propertyNode.fill(descriptor);
        }, this);
        // Object.freeze(value); ensure the object will not be modified
    ...
    compile() {
        ...
        for (let property of this) {
            // console.log('set property named', property.name, 'to', property.compile());
...
const ObjectPropertyElement = Element.extend('ObjectProperty', {
    ...
    set valueNode(element) {
        ...
        this.descriptor.value = element.value;
    },
    ...
    set getterNode(element) {
        this.appendChild(element);
        this.descriptor.get = element.value;
    },
    ...
    set setterNode(element) {
        this.appendChild(element);
        this.descriptor.set = element.value;
    },
    ...
    fill(descriptor) {
        ...
        if (descriptor.hasOwnProperty('value')) {
            this.valueNode = Lab.match(descriptor.value);
            this.valueNode.fill(descriptor.value);
        } else {
            if (descriptor.hasOwnProperty('get')) {
                this.getterNode = Lab.match(descriptor.get);
                this.getterNode.fill(descriptor.get);
            }
            if (descriptor.hasOwnProperty('set')) {
                this.setterNode = Lab.match(descriptor.set);
                this.setterNode.fill(descriptor.set);
            }
        }
    },
    ...
    compile() {
        // if (!this.valueNode) {
        //     console.log(this.value, 'has no valueNode', this.parentNode.children);
        // }
