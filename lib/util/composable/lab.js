...
ObjectElement.extend('Array', createConstructedByProperties(Array), {
    // array have obvisouly some specific behaviour at least regarding there merging strat
    write(value) {
        this.data = value.length;
        return ObjectElement.write.call(this, value);
    }
});
// handle function as primitive because perf and impossible to share scope
ObjectElement.extend('Function', createConstructedByProperties(Function), PrimitiveProperties);
// handle error as primitive because hard to share stack property
ObjectElement.extend('Error', createConstructedByProperties(Error), PrimitiveProperties);
ObjectElement.extend('RegExp', createConstructedByProperties(RegExp), {
    write(value) {
        this.data = value.valueOf();
        return ObjectElement.write.call(this, value);
    }
});
ObjectElement.extend('Date', createConstructedByProperties(Date), {
    write(value) {
        this.data = value.valueOf();
        return ObjectElement.write.call(this, value);
    }
});
// symbol are primitive
ObjectElement.extend('Symbol', PrimitiveProperties, {
    match(value) {
