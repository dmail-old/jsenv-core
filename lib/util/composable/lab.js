...
const ObjectElement = Element.extend('Object', {
    ...
    appendChild(child) {
        const currentProperty = this.getProperty(child.name);
        let returnValue;
        if (currentProperty) {
            const combinedProperty = currentProperty.createConstructor(child.name);
            // put the combined property at the right place
            returnValue = this.replaceChild(currentProperty, combinedProperty);
            // now combine the property
            combinedProperty.combine(currentProperty, child);
        } else {
            returnValue = ObjectElement.appendChild.call(this, child);
        }
        return returnValue;
    },
