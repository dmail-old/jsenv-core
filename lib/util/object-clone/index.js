...
function cloneValue(value, mergeObject, references) {
    var clonedValue;
    ...
    clonedValue = clone(value, mergeObject, references);
    return clonedValue;
}
...
function clonePropertyInside(emptyClone, origin, mergeObject, references, name) {
    ...
    var cloneMustCloneValueWithMergeObject;
        ...
        if (mergeObjectHasValue) {
            ...
            if (originHasValue) {
                ...
                if ((typeof originValue === 'object' && originValue !== null) || typeof originValue === 'function') {
                    ...
                    cloneMustCloneValueWithMergeObject = mergeObjectValue;
                } else {
                    cloneMustCloneValue = mergeObjectValue;
                    cloneMustCloneValueWithMergeObject = null;
                }
            } else {
                cloneMustCloneValue = mergeObjectValue;
                cloneMustCloneValueWithMergeObject = null;
    ...
    if (cloneMustHaveValue) {
        var clonedValue = cloneValue(cloneMustCloneValue, cloneMustCloneValueWithMergeObject, references);
...
function createEmptyClone(value/* , mergeValue */) {
...
function clone(value, mergeValue, references) {
    var clonedValue = createEmptyClone(value, mergeValue);
...
export const test = {
    ...
    main(assert) {
        ...
        // au vu des trois tests ci-dessous le second argument de clone doit être un objet options
        // avec quelques options comme
        // - listClonedProperties, 'auto', array, function returning 'auto' or an array of strings
        // - listMergedProperties, 'auto', array, function returning 'auto' or an array of strings
        // - mergedValue, any value used as mergedObject
        // - arrayConcat: Boolean, default to false
        // - arrayDeepClone: reverse of arrayConcat
        // the signature clone(a, b) will disapear in favor of clone(a, {mergedValue: b})
        // but we will provide a merge function that does that for us
        ...
        this.add("can concatenate array instead of merging them", function() {
            ...
            var b = ['b'];
            b.foo = true;
            var c = clone(a, b, true);

            // pour moi c'est une option dans le déroulement de clonePropertiesInside
            // qui doit donc clone tout les contenu du tableau a, puis y mettre le tableau b en ajoutant a.length
            // aux index de b
            // une autre option doit permettre de décider si oui ou non on clone les item dans les tableaux
            // ce serais par défaut à false lorsque concat vaut true
