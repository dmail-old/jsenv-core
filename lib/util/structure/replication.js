...
let ObjectReplicationProcess = (function() {
    ...
    let PropertyDefiner = util.createConstructor({
        constructor(replicationProcess, property, referenceMap) {
            var descriptor = {};
            var define;
            ...
            if (property.hasOwnProperty('valueDefinition')) {
                ...
                this.valueReplication = replicationProcess.createNested(
                ...
                if (hasGetter) {
                    this.getterReplication = replicationProcess.createNested(
                        property.getDefinition,
                        referenceMap
                    );
                }
                if (hasSetter) {
                    this.setterReplication = replicationProcess.createNested(
        ...
        replicate(replicationProcess, referenceMap) {
            let replica;
            if (referenceMap) {
                let reference = replicationProcess.reference;
                if (reference) {
                    replica = referenceMap.get(reference);
                } else {
                    replica = replicationProcess.replicate(referenceMap);
                }
            } else {
                replica = replicationProcess.replicate();
            }
            return replica;
        },

        definePropertyValue(value, referenceMap) {
            this.descriptor.value = this.replicate(this.valueReplication, referenceMap);
            Object.defineProperty(value, this.name, this.descriptor);
        },

        definePropertyGetter(value, referenceMap) {
            this.descriptor.get = this.replicate(this.getterReplication, referenceMap);
            Object.defineProperty(value, this.name, this.descriptor);
        },

        definePropertySetter(value, referenceMap) {
            this.descriptor.set = this.replicate(this.setterReplication, referenceMap);
            Object.defineProperty(value, this.name, this.descriptor);
        },

        definePropertyGetterAndSetter(value, referenceMap) {
            this.descriptor.get = this.replicate(this.getterReplication, referenceMap);
            this.descriptor.set = this.replicate(this.setterReplication, referenceMap);
            Object.defineProperty(value, this.name, this.descriptor);
        }
    ...
    let PropertiesDefiner = util.createConstructor({
        ...
        constructor(replicationProcess, definition, referenceMap) {
            if (definition.hasOwnProperty('properties')) {
                this.properties = definition.properties.map(function(property) {
                    let propertyDefiner = new PropertyDefiner(replicationProcess, property, referenceMap);
                    return propertyDefiner;
                }, this);
            }
        },

        define(value, referenceMap) {
            this.properties.forEach(function(property) {
                property.define(value, referenceMap);
            });
        }
    });
    ...
    let ObjectReplicationProcess = util.extendConstructor(ReplicationProcess, {
        ...
        populate(definition, referenceMap) {
            if (definition.references.length) {
                if (!referenceMap) {
                    // console.log(
                    //     'create a referencemap to collect self references and prevent their duplication for',
                    //     definition.value
                    // );
                    referenceMap = new util.ReferenceMap();
                }
                referenceMap.set(definition, this);
            } else {
                // console.log('no need for referencemap for', definition.value);
            }

            this.prototypeValue = Object.getPrototypeOf(definition.value);
            this.propertiesDefiner = this.createPropertiesDefiner(definition, referenceMap);
            this.guardDefiner = this.createGuardDefiner(definition);
        },

        createNested(definition, referenceMap) {
            let replication;

            // console.log('create a nested replication for', definition.value, Boolean(referenceMap));
            if (referenceMap) {
                let definitionReference = definition.reference;
                if (definitionReference) {
                    let existingReplicationForDefinition = referenceMap.get(definitionReference);
                    // console.log(
                    //     'the definition is a reference, searching for its replication',
                    //     Boolean(existingReplicationForDefinition)
                    // );
                    if (!existingReplicationForDefinition) {
                        throw new Error('cannot find the replication');
                    }
                    replication = new ReplicationProcess();// new reference.constructor();
                    // maybe new ReplicationProcess would be better in case the reference constructor does things
                    // a reference does nto have to do the same thing
                    replication.markAsReferenceTo(existingReplicationForDefinition);
                } else {
                ...
                let ReplicationConstructor = ReplicationProcessDatabase.match(definition);
                replication = new ReplicationConstructor();
                replication.populate(definition);
            }

            return replication;
        },

        createPropertiesDefiner(definition, referenceMap) {
            // ici on veut kkchose qui va définir toutes les propriétés sur le produit qu'on vient de créer
            var propertiesDefiner = new PropertiesDefiner(this, definition, referenceMap);
        ...
        // ptet avec le tableau references pour le coup
        // au lieu de retourner la référence dans createNestedReplication
        // on crée une replication avec une prop reference
        // et un tablea .references sur la référence d'origine
        // ensuite on génère une valeur et lorsqueu'ne replication
        // à un tableau de références, on le parcoure et on set pour chaque référence la dernière valeur générée
        // ensuite quand on tombe sur une replication ayant une prop .reference, on lit sa valeur .lastReferenceValue
        // cela évite une référenceMap mais ce n'est pas propre à cause de la propriété .lastReferenceValue
        // qui ne fonctionne que parce qu'on utilise pas deux fois le même objet replicationProcess
        // si replicationProcess utilise replicate() et que pendant le replicate() un autre replicate() survient
        // (peu propbable vu que c'est synchrone et quon n'aurait pas de moyen de faire ça à ma connaissance)
        // alors la propriété .lastReferenceValue serait fausse à un moment donné
        // pas le choix faut utiliser referenceMap
        replicate(referenceMap) {
            ...
            if (this.references.length) {
                ...
                referenceMap.set(this, value);
            }

            this.propertiesDefiner.define(value, referenceMap);
...
export const test = {
    ...
    main(assert) {
        ...
        this.add('cycle in properties', function() {
            ...
            value.self = value;
            let replication = scan(value);
            assert(replication.propertiesDefiner.properties[0].valueReplication.reference === replication);
