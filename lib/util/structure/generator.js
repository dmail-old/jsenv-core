...
(function() {
    ...
    // however primitive must remain the same, thanks to primitiveGenerator being the first trying to match it's ok
    [Boolean, Number, String].forEach(function(Constructor) {
        registerGeneratorForInstanceOf(Constructor, function(definition) {
            return this.prototypeValue.constructor(definition.value);
        });
    });
    ...
    // ideally we also need something like
