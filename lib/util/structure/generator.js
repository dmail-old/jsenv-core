...
// responsible to create raw product
// create the object owner of the properties
...
// responsible to transform the raw product into a semi-finished product
// will set properties & private properties of the object
let Transformer = util.createConstructor({
    constructor() {
...
// responsible to do any last transformation to make semi-finished product into a finished product
// will freeze object if needed
let Packager = util.createConstructor({
    constructor() {

    },

    pack(transformedValue) {
        return transformedValue;
    }
});

let CompiledGenerator = util.createConstructor({
    producer: null,
    transformer: null,
    packager: null,
    ...
    transform(rawValue) {
        return this.transformer.transform(rawValue);
    },

    pack(transformedValue) {
        return this.packager.pack(transformedValue);
    },

    generate() {
        let rawValue = this.produce();
        let transformedValue = this.transform(rawValue);
        let packedValue = this.pack(transformedValue);
        return packedValue;
    }
});

let CompilableGenerator = util.createConstructor({
    matcherConstructor: Matcher,
    producerConstructor: Producer,
    transformerConstructor: Transformer,
    packagerConstructor: Packager,

    constructor(phaseProperties = {}) {
        ['matcher', 'producer', 'transformer', 'packager'].forEach(function(name) {
            if (name in phaseProperties) {
                this[name + 'Constructor'] = util.extendConstructor(this[name + 'Constructor'], phaseProperties[name]);
            }
        }, this);

        this.matcher = new this.matcherConstructor(); // eslint-disable-line
    },
    ...
    compile(definition) {
        let compiledGenerator = new CompiledGenerator();

        compiledGenerator.producer = new this.producerConstructor(definition); // eslint-disable-line
        compiledGenerator.transformer = new this.transformerConstructor(definition); // eslint-disable-line
        compiledGenerator.packager = new this.packagerConstructor(definition); // eslint-disable-line

        return compiledGenerator;
    }
});

let GeneratorCompiler = {
    compilables: [],

    register(generatePhases = {}) {
        let compilableGenerator = new CompilableGenerator(generatePhases);
        this.compilables.push(compilableGenerator);

        return compilableGenerator;
    },
    ...
    compile(definition) {
        ...
        let compilableGenerator = this.compilables.find(function(compilableGenerator) {
            return compilableGenerator.match(definition);
        });

        if (!compilableGenerator) {
        ...
        // sinon faut instancier le compilableGenerator
        return compilableGenerator.compile(definition);
    }
};

function createConstructorMatcher(Constructor) {
    return {
...
function createLazyPrimitiveProducer() {
    return {
...
// il manque ceci (le plus important au final) mais sur le concept on est ok je pense
// je suis pas sur qu'on ai vraiment besoin de packager, le fait de freeze l'objet pourrait très bien se faire dans transformer
// c'est une fonctionnalité à part mais tout comme pirvate properties va faire partie de transformer
// on pourrait aussi y intégrer packager
let objectWithPropertiesTransformer = (function() {
    // let Definer = util.createConstructor();
    let CompiledProperty = util.createConstructor({
        ...
        compileDefiner(property) {
            ...
            if (property.hasOwnProperty('valueDefinition')) {
                ...
                define = definers.definePropertyValue;
            } else {
                ...
                if (hasGetter && hasSetter) {
                    define = definers.definePropertyGetterAndSetter;
                } else if (hasGetter) {
                    define = definers.definePropertyGetter;
                } else {
                    define = definers.definePropertySetter;
                }
            }
    ...
    return {
        compile(definition) {
            if (definition.hasOwnProperty('properties')) {
                this.properties = compileProperties(definition.properties);
            }
        }
    };
})();

let objectWithGuardPackager = (function() {
    let freezePackager = new Packager(function(semiFinishedProduct) {
        Object.freeze(semiFinishedProduct);
        return semiFinishedProduct;
    });
    let sealPackager = new Packager(function(semiFinishedProduct) {
        Object.seal(semiFinishedProduct);
        return semiFinishedProduct;
    });
    let preventExtensionPackager = new Packager(function(semiFinishedProduct) {
        Object.preventExtensions(semiFinishedProduct);
        return semiFinishedProduct;
    });
    ...
    return {
        compile(definition) {
            ...
            if (definition.hasOwnProperty('propertiesGuard')) {
                ...
                if (guard === 'frozen') {
                    packager = freezePackager;
                } else if (guard === 'sealed') {
                    packager = sealPackager;
                } else if (guard === 'non-extensible') {
                    packager = preventExtensionPackager;
                }
            }
            return packager;
        }
...
[
    ...
    GeneratorCompiler.register({
        ...
        transformer: objectWithPropertiesTransformer,
        packager: objectWithGuardPackager
    });
});
...
GeneratorCompiler.register({
    matcher: createConstructorMatcher(Object),
    producer: {
        constructor(definition) {
...
export default GeneratorCompiler;
