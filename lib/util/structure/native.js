...
/*
// c'est à la fois un component et un composite (comment exprimer ça?)
// étant donné que composite est optionel mais pas component il faut faire comme ça
const Properties = Element.compose();
Properties.define(Composite);
// Property est un component et aussi un Composite (il peut instancier des éléméents)
// mais on peut utiliser la propriété compose
const Property = Properties.compose();
Property.define(Composite);
Property.compose(Element);

// now we have established that an Element is composed of Properties composed of Property composed of Element
// we can define with more precision what is Properties & Property
// there is still much to do but we are closer to what we need
// oh and we got something bad here or at least inexact
// this is not ELement which are compose of Properties but ObjectElement
// and later I will need to reuse this structure for Entries so keep that reusability in mind
