...
ObjectElement.define({
    ...
    constructor() {
        this.properties = ObjectProperties.create(this);
        Element.constructor.apply(this, arguments);
    }
});

const ObjectProperties = LinkList.extend({
    initializer(objectElement) {
        const object = objectElement.value;
    ...
    createTargetLink(name) {
        return ObjectProperty.create(name);
    }
});

const ObjectProperty = Link.extend({
    name: '',
    attributes: {
        value: undefined,
        writable: true,
    ...
    initializer(object) {
        ...
        if ('value' in attributes) {
            this.add(ObjectPropertyValue.create(attributes.value));
            ...
            if (getter && setter) {
                this.add(ObjectPropertyGetter.create(getter));
                this.add(ObjectPropertySetter.create(setter));
            } else if (getter) {
                this.add(ObjectPropertyGetter.create(getter));
            } else {
                this.add(ObjectPropertySetter.create(setter));
...
const ObjectPropertyValue = Link.extend({
    constructor(objectProperty, value) {
        this.element = objectProperty.createTargetElement(value);
        this.element.link = this;
    }
});

const ObjectPropertyGetter = Link.extend({
    constructor(objectProperty, getter) {
        this.element = objectProperty.createTargetElement(getter);
        this.element.link = this;
    }
});

const ObjectPropertySetter = Link.extend({
    constructor(objectProperty, setter) {
        this.element = objectProperty.createTargetElement(setter);
        this.element.link = this;
    }
});
