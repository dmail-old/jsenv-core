...
const ObjectProperties = Constituent.extend({
    initializer(object) {
        ...
        propertyNames.forEach(function(name) {
            const propertyLink = this.createLink(name);
            this.add(propertyLink);
        }, this);
    },
    ...
    set: function(propertyName, value) {
        return this.defineProperty(this.element, propertyName, {
            configurable: true,
            enumerable: true,
            value: value,
            writable: true
    ...
    defineProperty(propertyName, attributes) {
        Object.defineProperty(this.element.value, propertyName, attributes);
        // create a property and add it
        return this.add(propertyName, attributes);
    },
    ...
    elementProperties: ['has', 'get', 'set', 'defineProperty', 'deleteProperty']
});

ObjectElement.Composition.add(ObjectProperties);

// may become a Composant as well
...
const ObjectGuard = Constituent.extend({
    initializer(element) {
    ...
    seal() {
        Object.seal(this.element.value);
        this.update('sealed');
    },

    freeze() {
        Object.freeze(this.element.value);
        this.update('frozen');
    },

    elementShortcut: 'guard',
    elementProperties: ['preventExtensions', 'seal', 'freeze']
});

ObjectElement.Composition.add(ObjectGuard);
