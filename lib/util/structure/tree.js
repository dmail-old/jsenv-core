...
const ObjectData = Data.extend({
    ...
    has(propertyName) {
        return this.links.some(function(link) {
            return ObjectProperty.isPrototypeOf(link) && link.name === propertyName;
        });
    },
...
const ObjectProperty = DataLink.extend({
    ...
    constructor(sourceData, name, attributes) {
        ...
        if ('value' in attributes) {
            ...
            this.value = this.createTargetData(attributes.value);
        } else {
            const getter = attributes.get;
            if (getter) {
                this.get = this.createTargetData(attributes.get);
            }
            const setter = attributes.set;
            if (setter) {
                this.set = this.createTargetData(attributes.set);
            }
        }
    },
