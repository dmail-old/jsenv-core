...
const ObjectLink = util.extend({
    ...
    addTarget(firstArg) {
        ...
        if (targetData.referencable) {
            const dataUsingSameValue = targetData.findPreviousDataByValue(targetData.value);
...
Data.define((function createNodeIterationMethods() {
    var properties = {
...
export const test = {
    ...
    main(assert) {
        this.add('manual object shadow', function() {
            ...
            let previousDatas = Array.from(barProperty.value.createPreviousIterable());
