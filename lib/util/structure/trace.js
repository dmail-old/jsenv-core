...
const ObjectProperty = util.extend({
    ...
    createTrace(value) {
        ...
        trace.link = ObjectPropertyLink.create(this, trace);
...
const ObjectPropertyLink = util.extend({
    constructor(trace, objectProperty) {
        this.trace = trace;
    ...
    getPrevious() {
        ...
        if (trace === property.value) {
            previousTrace = this.getPreviousPropertyDeepest();
        } else if (trace === property.get) {
            previousTrace = this.getPreviousPropertyDeepest();
        } else if (trace === property.set) {
            previousTrace = property.get.getDeepest() || property.get;
        }

        return previousTrace;
    },

    getPreviousPropertyDeepest() {
        const property = this.property;
        const propertyOwner = property.objectTrace;
        const ownerProperties = propertyOwner.properties;
        const propertyIndex = ownerProperties.indexOf(property);
        let deepestTrace;
        ...
        if (propertyIndex === 0) {
            ...
            deepestTrace = this.getPropertyDeepest(previousProperty);
        }

        return deepestTrace;
    },

    getPropertyDeepest(property) {
        let deepestTrace;

        if (property.hasOwnProperty('value')) {
            deepestTrace = property.value;
        } else if (property.hasOwnProperty('set')) {
            deepestTrace = property.set;
        } else {
            deepestTrace = property.get;
        }
    ...
    getDeepest() {
        ...
        while (true) { // eslint-disable-line
            ...
            const lastPropertyDeepestTrace = this.getPropertyDeepest(lastProperty);
...
// any trace may now have a link property used to link it to other trace
Trace.define({
    link: null,
    ...
    var properties = {
        createPreviousTraceIterable() {
            ...
            return createIterable(function() {
                let previousTrace = trace.getPrevious();
                trace = previousTrace;
        ...
        getPrevious() {
            return this.link.getPrevious();
        },

        getDeepest() {
            return this.link.getDeepest();
        }
    };
