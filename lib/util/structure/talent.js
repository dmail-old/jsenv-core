...
const Talent = {
    isPrototypeOf(value) {
        return TalentPrototype.isPrototypeOf(value);
    },
    ...
    create(definition) {
        ...
        const staticProperties = installProperties(talent, StaticProperties);
        staticProperties.collect(talent, talent);
        staticProperties.link(talent);

        const prototypeProperties = installProperties(talent, PrototypeProperties);
        prototypeProperties.collect(talent, talent);

        installProperties(talent, InstanceProperties);

        return talent;
    },

    link(talent, subject, ...args) {
        const prototypeProperties = talent.prototype;
        const instanceProperties = talent.instance;
        const subjectProperties = SubjectProperties.create();

        const linkedTalent = Object.create(talent);
        instanceProperties.collect(linkedTalent, ...args);
...
Object.assign(Talent, {
    compose(...talents) {
        const compositeTalent = Object.create(TalentPrototype);

        const compositeStaticProperties = StaticProperties.create();
        compositeStaticProperties.compose(...talents.map(function(talent) {
...
Talent.resolve = function(talent, resolvers) {
    ...
    if (hasPrototype || hasInstance) {
        resolvedTalent = Object.create(TalentPrototype);
        resolvedTalent.static = talent.static;
        resolvedTalent.static.link(resolvedTalent);

        if (hasPrototype) {
            resolvedTalent.prototype = talent.prototype.resolve(resolvers.prototype);
        } else {
            resolvedTalent.prototype = talent.prototype;
        }
        if (hasInstance) {
            resolvedTalent.instance = talent.instance.resolve(resolvers.instance);
        } else {
            resolvedTalent.instance = talent.instance;
        }
    } else {
...
// prototype properties can be composed, only them can be (done during Talent.compose)
// during compose we must detect conflict and try to resolved them shouldn't we?
    ...
    main(assert) {
        this.add('Talent.create', function() {
            this.add('static only', function() {
                ...
                assert(talent.foo() === 'foo');
                assert(Talent.isPrototypeOf(talent));
        ...
        this.add('Talent.resolve', function() {
            ...
            const resolvedTalent = Talent.resolve(talent, {
                prototype: {
                    foo: 'replace',
                    test: {
                        rename: 'free'
        ...
        this.add('Talent.link', function() {
            this.add('static only', function() {
                ...
                const subject = {};
                const link = Talent.link(talent, subject);
            ...
            this.add('prototype only', function() {
                ...
                Talent.link(talent, secondSubject);
                ...
                const talent = Talent.create({
                    ...
                    instance(name) {
                        ...
                        return {
                            ...
                            protoFoo() {
                                return talent.prototype.call('foo', this);
                            }
                        };
                    }
                });
