...
export const test = {
    ...
    main(assert) {
        ...
        this.add('talent can be composed together', function() {
            // ensure all talent properties are correctly set
        });
        ...
        this.add('property collision creates an error during talent compilation', function() {
            ...
            // every strategy got his own story, renaming, ignoring, replacing etc...
        });
        ...
        this.add('Talent.resolve', function() {
            this.add('basic resolve', function() {
                ...
                assert(barProperty.resolver.name === 'inherit');
                const fooProperty = resolvedTalent.prototypeProperties.get('foo');
                assert(fooProperty.resolver.name === 'replace');
                // ensure test was removed because renamed into free which leaded to its removal
                assert(resolvedTalent.prototypeProperties.has('test') === false);
                assert(resolvedTalent.prototypeProperties.has('free') === false);
            ...
            this.add('resolve + link on prototype conflict', function() {
                const protoModel = {
                    ...
                    willBeReplaced() {},
                    willBeIgnored() {},
                    willBeExecutedAfter: spy(),
                    willBeExecutedBefore: spy(),
                    willBeExecutedAround: spy()
                };
                const talent = Talent.create({
                    prototype() {
                        return protoModel;
                    ...
                    instance(local, prototype) {
                        return {
                            protoNewName: prototype.newName,

                            callProtoAround() {
                                return prototype.willBeExecutedAround.apply(this, arguments);
                            }
                        };
                    }
                });
                const aroundSpy = spy(function(previous, current) {
                    previous();
                    current();
                });
                const resolvedTalent = talent.resolve({
                ...
                // ensure prototype passed to instance is already renamed
                assert(subject.protoNewName === protoModel.willBeRenamed);
                ...
                assert(subject.willBeIgnored === subjectModel.willBeIgnored);
                subject.willBeExecutedAfter();
                assertCallOrder(subjectModel.willBeExecutedAfter, protoModel.willBeExecutedAfter);
                subject.willBeExecutedBefore();
                assertCallOrder(protoModel.willBeExecutedBefore, subjectModel.willBeExecutedBefore);
                // calling subject.callProtoAround allow to ensure that prototype.willBeExecutedAround passed to instance
                // is already the composition of calling subject.willBeExecutedAround, then protoModel.willBeExecutedAround
                subject.callProtoAround('test');
                assertCallOrder(aroundSpy, subjectModel.willBeExecutedAround, protoModel.willBeExecutedAround);
                assert(aroundSpy.lastCall.args[2][0] === 'test');
            });
        });
...
const Resolver = (function() {
    ...
    const InheritResolver = PropertyResolver.extend({
        name: 'inherit'
    });
    Resolver.register(InheritResolver);
