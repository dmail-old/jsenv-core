...
const Properties = util.extend({
    ...
    define(subject) {
        for (let property of this) {
            property.assign(subject).define();
        }
    }
});
...
const Property = util.extend({
    ...
    // link(owner) {
    //     // console.log('link property', this.name, 'on', owner, 'with descriptor', this.descriptor);
    //     const assignedProperty = this.assign(owner);
    //     assignedProperty.define();
    //     const link = {
    //         property: this,
    //         unlink() {
    //             this.property.link(this.property.owner); // redefine property on the owner
    //         }
    //     };
    //     return link;
    // }
...
Property.refine({
    detectResolutionConflictWith(otherProperty) {
        ...
        const otherStrategy = otherProperty.resolutionStrategy;

        if (selfStrategy.name === 'default' && otherStrategy.name === 'default') {
            throw new Error('conflict between default resolution strategy for property named "' + this.name + '"');
        }

        if (selfStrategy.name === 'rename' && otherStrategy.name === 'rename') {
            if (selfStrategy.renameWith === otherStrategy.renameWith) {
                throw new Error('conflict between rename resolution strategy for property named "' + this.name + '"');
            }
        }

        if (selfStrategy.name === otherStrategy.name) {
            throw new Error(
                'conflict between ' + selfStrategy.name + ' resolution strategy for property named "' + this.name + '"'
            );
        }
    }
});
