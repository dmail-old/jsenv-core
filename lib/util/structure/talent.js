...
Talent.refine({
    ...
    collectInstanceProperties(subject, ...args) {
        ...
        // we have to create the subjectProperties here because of composition which call directly this
        // method but it's very bad for perf and we uselessly recollect all subject properties
        // when we already know them
        const subjectProperties = this.collectSubjectProperties(subject);
        const delegatedPrototypeProperties = Properties.create();
        delegatedPrototypeProperties.merge(this.prototypeProperties);
        delegatedPrototypeProperties.combine(subjectProperties);
        const delegatedPrototype = this.createPropertiesTarget(delegatedPrototypeProperties);
        // for now we create a fresh prototype object per call to collectInstanceProperties()
        ...
        instanceProperties.populate(this, Object.create(this.local), delegatedPrototype, subject, ...args);
