...
Talent.refine({
    link(subject, ...args) {
        const subjectProperties = this.collectSubjectProperties(subject, ...args);
    ...
    collectSubjectProperties(subject, ...args) {
    ...
    resolveProperty(property, conflictResolution) {
        ...
        if (conflictResolution.hasOwnProperty(propertyName)) {
            ...
            if (resolvedProperty.resolutionStrategy.name === 'rename') {
                ...
                const conflictualProperty = this.get(renamedWith);
                if (conflictualProperty) {
                    let error = property.createConflictError(
                        conflictualProperty,
                        'conflict must not be handled by renaming "' + property.name + '" -> "' + renamedWith + '" because it already exists',
                        'resolve({rename: \'' + renamedWith + '-free\'})'
                    );
                    throw error;
                }
    ...
    compose(...talents) {
        ...
        try {
            ...
            if (e.name === 'PropertyError') {
                const property = e.meta.property;
                const conflictualProperty = e.meta.conflictualProperty;
                const name = property.name;
                const allTalents = [this, ...talents];
                const firstTalentUsingProperty = allTalents.find(function(talent) {
                    return talent.prototypeProperties.get(name) === conflictualProperty;
                });
                const firstIndex = allTalents.indexOf(firstTalentUsingProperty);
                const nextTalents = allTalents.slice(firstIndex - 1);
                const secondTalentUsingProperty = nextTalents.find(function(talent) {
                    return talent.prototypeProperties.get(name) === property;
                });
                const secondIndex = allTalents.indexOf(secondTalentUsingProperty);
                ...
                message += firstTalentName + ' & ' + secondTalentName;
                message += ' have a conflict in one of their prototype properties';

                const talentCompositionError = new Error(message);

                talentCompositionError.hint = 'You may want to resolve conflict using';
                talentCompositionError.detail = e.message;
                var howToFix = e.meta.howToFix;
                howToFix = howToFix.slice('resolve('.length, -1);
                talentCompositionError.hintSource = 'talent.resolve({prototype: {' + name + ': ' + howToFix + '}})';
        ...
        for (let properties of propertiesList) {
            for (let property of properties) {
                ...
                if (currentProperty) {
                    ...
                    if (
                        currentProperty.hasOwnProperty('resolutionStrategy') &&
                        currentProperty.resolutionStrategy.name !== 'default'
                    ) {
...
Property.refine({
    detectResolutionConflictWith(otherProperty) {
        ...
        if (selfStrategy.isConflictualWithStrategyOfProperty(otherProperty.resolutionStrategy)) {
