...
const Skill = util.extend({
    ...
    constructor() {
        ...
        if (this.hasOwnProperty('properties')) {
            this.properties = createPropertiesFromObject(this.properties);
        }
...
const Talent = util.extend({
    ...
    add() {
        ...
        this.assignProperties(skill.properties);

        const dynamicPropertiesConstructor = skill.dynamicProperties;
        if (dynamicPropertiesConstructor) {
            const dynamicPropertiesObject = dynamicPropertiesConstructor.apply(this, arguments);
            const dynamicProperties = createPropertiesFromObject(dynamicPropertiesObject);
            this.assignProperties(dynamicProperties);
        }

        const selfProperty = skill.selfProperty;
        if (selfProperty) {
            // it may conflict with other properties, in such case we would throw
            const skillAssignment = Assignment.create(this.subject, selfProperty.define({value: this}));
...
const Assignment = util.extend({
    ...
    save() {
        ...
        if (target.hasOwnProperty(name)) {
            previous = this.createConstructor(
                target,
                this.property.define(Object.getOwnPropertyDescriptor(target, name)),
                true
            );
        } else {
            previous = this.createConstructor(target, this.property.delete(), true);
        }
...
const SkillList = Skill.extend({
    ...
    createTalent(subject) {
        return TalentList.create(this, subject);
    }
});

const TalentList = util.extend({
    constructor(skillList, subject) {
        this.talents = skillList.skills.map(function(skill) {
            return skill.createTalent(subject);
        });
        this.add();
    },
            ...
            const randomItemSkill = (function() {
                ...
                return Skill.create({
                    dynamicProperties(list) {
                        return {
                            item(index) {
                                return list[mathFloor(parseFloat(index, 10))];
                            }
                        };
                    }
                });
