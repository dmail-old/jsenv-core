...
const Properties = util.extend({
    ...
    concat(...propertiesList) {
        ...
        Object.assign(concatenedProperties.strategy, this.strategy);

        for (let properties of propertiesList) {
            Object.assign(concatenedProperties.map, properties.map);
            Object.assign(concatenedProperties.strategy, properties.strategy);
        }
    ...
    resolveProperty(property) {
        ...
        const selfProperty = this.get(propertyName);
        let resolvedProperty;
        if (selfProperty) {
            const resolver = this.getResolver(propertyName);
            resolvedProperty = resolver.call(this, selfProperty, property);
        } else {
    ...
    link(subject) {
        ...
        for (let property of this) {
            let resolvedProperty;
            if (subject.hasOwnProperty(property.name)) {
                resolvedProperty = this.resolveProperty(property);
                // pas bon, on check pas de la même manière, resolveProperty cherche dans this.properties
                // alors qu'il faudrais cherche dans subject, et en plus il faudrais que lorsque resolver
                // apelle resolveProperty il cherche dans subject et pas dans this
            } else {
                resolvedProperty = property;
            }

            const propertyLink = resolvedProperty.define(subject);
            if (propertyLink) {
...
const resolvers = {
    unexpectedString(property) {
        throw new Error(
            'conflict resolver for property ' + property.name +
            ' string must be oneOf replace,remove,before,after'
        );
    },
    unexpectedObject(property) {
        throw new Error(
            'conflict resolver for property ' + property.name +
            ' object must have oneOf rename,around properties'
        );
    },
    unexpectedValue(property) {
        throw new Error(
            'conflict resolver for property ' + property.name +
    ...
    around(around) {
        return function(property, conflictualProperty) {
    ...
    after(property, conflictualProperty) {
    ...
    before(property, conflictualProperty) {
        return property.set(composeFunction(
            property.attributes.value,
            conflictualProperty.attributes.value,
            'before'
    ...
    remove(property) {
        // ignore conflictualProperty, kepp property
        return property;
    },
    replace(property, conflictualProperty) {
        // replace property by conflictualProperty
        return conflictualProperty;
    },
    rename(renameWith) {
        return function(property) {
            const renamedProperty = property.rename(renameWith);
            // renamedProperty may confict as well so we have to recursively check for renamed property conflict
            return this.resolveProperty(renamedProperty);
        };
    }
};
...
const TalentList = util.extend({
    constructor(...talents) {
        ...
        this.prototype = PrototypeProperties.create().compose(...talents.map(function(talent) {
