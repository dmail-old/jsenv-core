...
Properties.refine({
    ...
    resolveProperty(property, conflictResolution) {
        ...
        if (conflictResolution.hasOwnProperty(propertyName)) {
            ...
            const resolutionStrategy = Resolution.from(conflictResolution[propertyName]);
            if (!resolutionStrategy) {
                throw new Error(
                    'no resolution strategy registered matched ' +
                    conflictResolution[propertyName] + ' for property named "' + propertyName + '"'
                );
            }

            resolvedProperty = resolutionStrategy.resolveNow(property, this, conflictResolution);
        } else {
...
const Resolution = (function() {
    ...
    const PropertyResolutionStrategy = util.extend({
        ...
        resolveNow(property) {
            ...
            resolvedProperty.resolutionStrategy = this;
            return resolvedProperty;
        },
    ...
    const ReplaceResolutionStrategy = PropertyResolutionStrategy.extend({
        name: 'replace',
        resolveLater(property, conflictualProperty) {
            if (conflictualProperty.resolutionStrategy.name === 'replace') {
    ...
    const RenameResolutionStrategy = PropertyResolutionStrategy.extend({
        ...
        resolveNow(property, properties, resolution) {
            let resolvedProperty;
            ...
            if (conflictualProperty) {
                ...
                resolvedProperty = properties.resolveProperty(renamedProperty, resolution);
            }
...
Property.refine({
    resolveConflict(conflictualProperty) {
        ...
        const resolvedProperty = propertyResponsibleToResolve.resolutionStrategy.resolveLater(
