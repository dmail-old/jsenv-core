...
const Talent = util.extend({
    constructor(definition) {
        ...
        const localProperties = installProperties(this, LocalProperties);
        localProperties.collect(this);
        localProperties.object = {};
        localProperties.link(localProperties.object);

        const prototypeLocalProperty = localProperties.get('prototype');
        if (prototypeLocalProperty) {
            prototypeLocalProperty.define(this);
        }
        const prototypeProperties = installProperties(this, PrototypeProperties);
        prototypeProperties.collect(this, localProperties.object);
        prototypeProperties.object = {};
        prototypeProperties.link(prototypeProperties.object);

        const instanceLocalProperty = localProperties.get('instance');
        if (instanceLocalProperty) {
            instanceLocalProperty.define(this);
        }
        installProperties(this, InstanceProperties);
    },
...
Talent.define({
    link(subject, ...args) {
        const localProperties = this.local;
        const prototypeProperties = this.prototype;
        const instanceProperties = this.instance;
        const subjectProperties = SubjectProperties.create();

        const linkedTalent = Object.create(this);
        instanceProperties.collect(linkedTalent, localProperties.object, prototypeProperties.object, ...args);
...
Talent.define({
    resolve(resolvers) {
        ...
        if (hasPrototype || hasInstance) {
            ...
            resolvedTalent.local = this.local;
...
Talent.define({
    compose(...talents) {
        ...
        compositeTalent.local = compositeLocalProperties;
    ...
    main(assert) {
        this.add('Talent.create', function() {
            this.add('local only', function() {
                ...
                assert(talent.local.get('foo').name === 'foo');
