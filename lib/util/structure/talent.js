...
Talent.resolve = function(talent, resolvers) {
    ...
    if (hasPrototype || hasInstance) {
        // right now the resolvedTalent is just an instance of the talent with different properties
        // maybe we should clone the talent instead of doing Object.create()
        // like for instance doing resolvedTalent = Object.create(TalentPrototype);
        // resolvedTalent.static = talent.static;
        // resolvedTalent.prototype = talent.prototype;
        // resolvedTalent.instance = talent.instance;
        // talent.static.define(resolvedTalent);
...
// before being defined on an object PrototypeProperties are merged with instanceProperties
// then they will be set on the subject
PrototypeProperties.define({
    merge(instanceProperties) {
        const subjectProperties = SubjectProperties.create();
        Object.assign(subjectProperties.map, this.map, instanceProperties.map);
