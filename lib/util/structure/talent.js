...
Fixed, Shared, Exposed, Privilegied, Prototype, Delegate, Import
...
/*
Ok donc l'idée c'est de faire ce que fait stampit en différent pour bien se l'approprier
un truc que fait stamp que je kiff c'est de créer un stamp à chaque mutation de sorte qu'on obtient
l'approche talent consiste à vouloir mettre le talent sur un object existant
là ou stampit va juste appeler stamp() pour créer l'objet que l'on souhaite je pense qu'on va utiliser ça
il faut aussi choisir un style alors je propose plusieurs examples pour illustrer le délire
trouver un nom j'apellerais bien ça unit parce que les talents s'installent sur des object existant
alors que là on compose des unités de comportement réutilisable

private -> implement(), conceal(), hide(), veil(), define(), refine(), specify(), describe(), improve(), merge(), concat()

What I want to achieve               | using this               | using class              | using stampit
------------------------------------ | ------------------------ | ------------------------ | --------------
static property declaration          | implement({foo: true})   | static foo = true        | staticProperties({foo: true})
static method declaration            | implement({bar() {}})    | static bar() {}          | statics(bar() {})
public static property declaration   | expose({foo: true})      | public static foo = true | props({foo: true})
public static method declaration     | expose({bar() {}})       | public static bar() {}   | method({bar() {}})
public property declaration          | this.expose() inside init| public foo = true        | assign this inside init
public method declaration            | this.expose() inside init| public bar() {}          | assign this inside init
static access from outside           | part.internal            | ClassName::foo           | stamp.foo
public static access from outsite    | part.exposed             | impossible               | stamp.fixed.methods.bar
instance creation                    | part({})                 | new ClassName(a, b)      | stamp({})
overrides instancied value           | return inside init       | return inside constructor| return iniside init
args access from constructor         | options arg in init()    | arg passed to constructor| first arg of init()
static acccess from constructor      | internal arg in init()   | className->foo           | init(opts, {stamp}) stamp.foo
public static acess from constructor | exposed arg in init()    | className::foo           | init(opts, {stamp}) stamp.fixed.methods.bar
constructor declaration              | init(fn)                 | constructor() {}         | init(fn)
configuration declaration            | to be defined            | not supported            | conf({foo: true})
configuration access from constructor| to be defined            | not supported            | init(opts, {stamp}) stamp.compose.configuration.foo

// https://github.com/stampit-org/stampit/blob/master/src/compose.js

*/

/*
const part = unit().implement({
    method() {
        return 'ok';
    }
}).expose({
...
export const test = {
    ...
    main(assert) {
        // here we should have access by default to this file exports
        ...
        function assertCallOrder(...spies) {
            spies.reduce(function(previous, spy) {
                assert(previous < spy.lastCall.id);
                return spy.lastCall.id;
            }, -1);
        }

        this.add('properties are created inside a function to benefit from scope', function() {
            assert.throws(function() {
                Talent.create(null);
            }, function(e) {
                return e.name === 'TypeError' && e.message === 'Talent constructor first argument must be a function';
            });

            const executor = spy(function() {
                return {
                    method() {}
                };
            });
            const talent = Talent.create(executor);
            assert(executor.lastCall.this === talent);
            assert(executor.lastCalll.args.length === 0);
            assertMethods(executor.lastCall.return, talent);
        });

        this.add('talent prototype/constructor core mecanisms are mirrored using local/instance method', function() {
            // prototype core mecanism : you can create prototype properties
            // constructor core mecanism: a function is called with a fresh object holding the prototype properties on instantiation
            // we don't set local on talent to avoid possible conflict with talent API such as compose/resolve
            ...
            const executor = spy(function() {
                return {
                    ...
                    instance: spy(function(local) {
                        local.instancied = true;
                    })
                };
            });
            const talent = Talent.create(executor);
            const localSpy = executor.lastCall.return.local;
            const localCall = localSpy.lastCall;
            const instanceSpy = executor.lastCall.return.instance;
            const instanceCall = instanceSpy.lastCall;
            assert(instanceCall.called === false);

            talent.compile(); // this is how talent will be instancied
            let firstCompileInstanceFirstArg = instanceCall.args[0];
            assertMethods(localCall.return, instanceCall.args[0]);
            talent.compile();
            assertMethods(localCall.return, instanceCall.args[0]);
            assert(firstCompileInstanceFirstArg !== instanceCall.args[0]);
        });

        this.add('prototype & instance properties of compiled talent can be installed on object', function() {
            const talent = Talent.create(function() {
                var commonPrivate = 'Hello ';

                return {
                    local() {
                        var localPrivate = 'local';

                        return {
                            method: spy(function() {
                                return commonPrivate + localPrivate;
                            })
                        };
                    },
                    ...
                    prototype() {
                        ...
                        return {
                            method: spy(function() {
                                return commonPrivate + prototypePrivate;
                            })
                        };
                    },

                    instance(local, prototype, subject, name) {
                        var instancePrivate = 'instance';

                        return {
                            localMethod: local.method,
                            protoMethod: prototype.method,
                            method: spy(function() {
                                return commonPrivate + instancePrivate;
                            }),
                            [subject.name]() {},
                            [name]() {}
                        };
            ...
            const target = {};
            const compiled = talent.compile(subject, 'test');
            const installer = compiled.install(target);

            target.method.call(subject);
            // assert method are not bound
            assert(target.method.lastCall.this === subject);
            assert(target.method() === 'Hello instance');
            assert(target.method.lastCall.this === target);
            assert(target.protoMethod() === 'Hello prototype');
            assert(target.protoMethod.lastCall.this === target);
            assert(target.localMethod() === 'Hello local');
            // maybe localMethod should be bound to talent or something because
            // they are specific to the talent and are not meant to be delegated
            assert(target.localMethod.lastCall.this === target);
            assert(typeof target.test === 'function');
            assert(typeof target.subject === 'function');

            installer.uninstall();
            assert(Object.keys(target).length === 0);
        });

        this.add('property collision creates an error during talent compilation', function() {
            // ya un truc a clarifier : est-ce qu'on détecte les conflit entre target et talent properties ?
            // subject c'est sur, on résolve même par rapport à lui
            // par contre target est ce qu'on veut détecter ces erreurs ?
            // parce que around etc sont résolus par rapport à subject
            // j'ai aussi un autre souci : si subject a une propriété mais target
            // ne l'a pas on va chercher à résoudre un conflit qui n'existera pas en définitive
             /*
            clarifier aussi si on peut écrire
            instance(local, prototype, subject) {
                return {
                    method() {
                        return subject.name;
                    }
            ...
            puisque du coup la méthod sera bound à subject
            mais dans prototype ce ne sera pas le cas ce qui peut perturber
            je suis pas sur qu'on ai besoin de pouvoir apeller method.call avec ot chose
            */

            const prototypeMethod = spy();
            const subjectMethod = spy();
            const prototypeCall = prototypeMethod.lastCall;
            const subjectCall = subjectMethod.lastCall;
            ...
            assert.throws(function() {
                talent.install(subjectModel);
            }, function(e) {
                return e.name === 'Error';
            });

            this.add('you can resolve conflict by renaming', function() {
                this.add('renaming throw an error when it results in an other property collision', function() {
                ...
                this.add('renaming propagate the resolver', function() {
                    const subject = Object.assign({}, subjectModel);
                    talent.resolve({
                        prototype: {
                            method: {
                    ...
                    assert(subject.method3 === prototypeMethod);
                    assert(subject.method2 === undefined);
                    assert(subject.method === subjectMethod);
                });
            });

            this.add('you can resolve conflict by ignore', function() {
                // ignore does nto remove the property but skip it
                // it's important not to remove the property from prototype
                // else we loose the property information and we cannot uninstall them later
                const subject = Object.assign({}, subjectModel);
                talent.resolve({
                    prototype: {
                        method: 'ignore'
            ...
            this.add('you can resolve conflict by replace', function() {
                const subject = Object.assign({}, subjectModel);
                talent.resolve({
                    prototype: {
                        method: 'replace'
                ...
                assert(subject.method === prototypeMethod);
            });

            this.add('you can resolve conflict by around', function() {
                const subject = Object.assign({}, subjectModel);
                const around = spy();
                talent.resolve({
                    prototype: {
                ...
                const aroundCall = around.lastCall;
                assert(subject.method === around);
                subject.method('test');
                assert(aroundCall.args[0] === subjectMethod);
                assert(aroundCall.args[1] === prototypeMethod);
                assert(aroundCall.args[2][0] === 'test');
                assert(aroundCall.this === subject);
            });

            this.add('you can resolve conflict by before', function() {
                const subject = Object.assign({}, subjectModel);
                talent.resolve({
                    prototype: {
                        method: 'before'
                ...
                subject.method('test');
                assertCallOrder(prototypeMethod, subjectMethod);
                assert(subjectCall.this === subject);
                assert(subjectCall.args[0] === 'test');
                assert(prototypeCall.this === subject);
                assert(prototypeCall.args[0] === 'test');
            });

            this.add('you can resolve conflict by after', function() {
                const subject = Object.assign({}, subjectModel);
                talent.resolve({
                    prototype: {
                        method: 'after'
                ...
                subject.method('test');
                assertCallOrder(subjectMethod, prototypeMethod);
                assert(subjectCall.this === subject);
                assert(subjectCall.args[0] === 'test');
                assert(prototypeCall.this === subject);
                assert(prototypeCall.args[0] === 'test');
            ...
            this.add('you can unresolve conflict using initial', function() {
                // what we really want with initial
                // is to restore the property resolver to the default one
                // the default one being the one who throw but it may be an other one
                const subject = Object.assign({}, subjectModel);
                assert.throws(function() {
                    talent.resolve({
                        prototype: {
        ...
        this.add('talent can be composed together', function() {
            const firstSpy = spy(function() {
                return {
                    local: spy(function() {
                        return {
                            method() {}
            ...
            const firstTalent = Talent.create(firstSpy);
            ...
            const subject = {};
            const installer = compositeTalent.install(subject, 'test');

            [firstTalent, secondTalent].forEach(function(talent) {
                const spy = talent === firstTalent ? firstSpy : secondSpy;
                const instanceCall = spy.instance.lastCal;

                assert(instanceCall.this === talent);
                assertMethods(instanceCall.args[0], spy.local.lastCall.return);
                assertMethods(instanceCall.args[1], spy.prototype.lastCall.return);
                assert(instanceCall.args[2] === subject);
                assert(instanceCall.args[3] === 'test');
                assertMethods(instanceCall.return, subject);
            });
        ...
        this.add('property collision creates an error for composed talent', function() {
            this.add('prototype properties conflict throw on composition', function() {

            });

            this.add('instance properties conflict throw on compilation with specific error message', function() {
...
Talent.refine({
    link(subject, ...args) {
        const compiled = this.compile(subject, ...args);
        const installer = compiled.createInstaller(subject);
        installer.install();
        return installer;
    },

    compile(subject, ...args) {
        // the properties of the subject object
        const subjectProperties = Properties.create();
        ...
        // what we wnat first is to handle conflict between subject & prototype properties
        // but nothing more after that we just want to keep that list of prototype properties to
        // merge it with instance properties
        const collectedPrototypeProperties = this.collectPrototypeProperties();
        const resolvedPrototypeProperties = collectedPrototypeProperties.resolveAgainst(subjectProperties);

        // after what we want is to create a prototype from the resolvedPrototypeProperties
        // and get a resolvedInstanceProperties
        const collectedInstanceProperties = this.collectInstanceProperties(
            resolvedPrototypeProperties, subject, ...args
        );
        const resolvedInstanceProperties = collectedInstanceProperties.resolveAgainst(subjectProperties);

        // now we have resolvedPrototypeProperties & resolvedInstanceProperties we can merge them
        // (instance properties overrides prototype properties)
        const sharedProperties = resolvedPrototypeProperties.clone().merge(resolvedInstanceProperties);

        const compiled = {
            createInstaller(target) {
                let targetProperties;
                if (target === subject) {
                    targetProperties = subjectProperties;
                } else {
                    targetProperties = Properties.create();
                    targetProperties.populateByObject(target, true);
                }
                // to be able to revert changes
                const diffProperties = sharedProperties.diff(targetProperties);

                // using a separated object (installer) prevent memory from keeping in memory how to
                // remove a talent properties when nothing keep a reference to it
                const installer = {
                    install() {
                        sharedProperties.define(target);
                    },

                    uninstall() {
                        diffProperties.define(target);
                    }
                };
    ...
    proxy(proxy, subject, ...args) {
        const compiled = this.compile(subject, ...args);
        const installer = compiled.createInstaller(proxy);
        installer.install();
        return installer;
    },

    collectPrototypeProperties() {
        return this.prototypeProperties;
    },

    collectInstanceProperties(resolvedPrototypeProperties, subject, ...args) {
        // for now we create a fresh prototype object per call to populateSubjectInstanceProperties()
        // we have to do this in case prototypeProperties contains property which use
        // oneOf around, before, after resolutionStrategy
        // because thoose resolutionStrategy have to know the property they conflict with
        const prototype = this.createPropertiesTarget(resolvedPrototypeProperties);

        const collectedInstanceProperties = this.instanceProperties.clone();
        collectedInstanceProperties.populate(
            this,
            Object.create(this.local),
            prototype,
            subject,
            ...args
        );
        return collectedInstanceProperties;
    }
});
...
Properties.refine({
    ...
    resolveAgainst(properties) {
        const resolvedProperties = this.createConstructor();

        for (let property of this) {
            const propertyName = property.name;
            const conflictualProperty = properties.get(propertyName);

            if (conflictualProperty) {
                // detect if both resolutionStrategy are conflictual
                // console.log('want to resolve', property.source, 'and', currentProperty.source);
                const resolvedProperty = property.resolveConflict(conflictualProperty);
                // console.log('the resolved is', resolvedProperty.source);
    ...
    diff(properties) {
        var diffProperties = Properties.create();
...
const Resolver = (function() {
    ...
    const InitialResolver = PropertyResolver.extend({
        name: 'initial',
        resolveNow(property) {
            let resolvedProperty = property.clone();
    ...
    Resolver.register(InitialResolver);
    ...
    const AroundResolver = PropertyResolver.extend({
        name: 'around',
        constructor(around) {
            this.around = around;
        },
    ...
    Resolver.register(AroundResolver);

    const AfterResolver = PropertyResolver.extend({
        name: 'after',
        resolveLater(property, conflictualProperty) {
            return property.set(composeFunction(
                conflictualProperty.descriptor.value,
    ...
    Resolver.register(AfterResolver);

    const BeforeResolver = PropertyResolver.extend({
        name: 'before',
        resolveLater(property, conflictualProperty) {
            return property.set(composeFunction(
                conflictualProperty.descriptor.value,
    ...
    Resolver.register(BeforeResolver);

    const RemoveResolver = PropertyResolver.extend({
        name: 'remove',
        resolveNow(property) {
            return property.delete();
        }
    });
    Resolver.register(RemoveResolver);

    const IgnoreResolver = PropertyResolver.extend({
        name: 'ignore',
        resolveLater(property, conflictualProperty) {
            return conflictualProperty;
        }
    });
    Resolver.register(IgnoreResolver);

    const ReplaceResolver = PropertyResolver.extend({
        name: 'replace',
        resolveLater(property, conflictualProperty) {
    ...
    Resolver.register(ReplaceResolver);

    const RenameResolver = PropertyResolver.extend({
        name: 'rename',
    ...
    Resolver.register(RenameResolver);
...
Talent.refine({
    compose(...talents) {
        ...
        //     // le populate ici est particulier, on va avoir besoin de récup chaque instance properties de chaque talent
        //     // afin de pouvoir faire ça on a besoin de générer aussi le prototype properties
        //     // puisqu'on a besoin d'apeller avec un prototype particulier

        //     // j'ai besoin du sharedPrototypeProperties, ce que je n'ai pas ici
        ...
        // compositeTalent.collectSharedProperties = function(subject, ...args) {
        //     const instanceProperties = old.instanceProperties.clone();

        //     // get the properties shared generically
        //     const compositeSharedPrototypeProperties = subjectProperties.clone();
        //     const compositeSharedInstanceProperties = subjectProperties.clone();
        //     const compositeSharedDiffProperties = Properties.create();

        //     const collectedPrototypeProperties = old.collectPrototypeProperties();
        //     const sharedPrototypeDiff = sharedPrototypeProperties.combine(collectedPrototypeProperties);
        //     compositeSharedDiffProperties.merge(sharedPrototypeDiff);
        //     const collectedInstanceProperties = old.collectInstanceProperties(sharedPrototypeProperties, subject, ...args);
        //     const sharedInstanceDiff = sharedInstanceProperties.combine(collectedInstanceProperties);
        //     compositeSharedDiffProperties.merge(sharedInstanceDiff);
    ...
    combine(properties) {
        ...
        for (let property of properties) {
            ...
            if (currentProperty) {
                ...
                // console.log('the resolved is', resolvedProperty.source);
                if (currentProperty !== resolvedProperty) {
