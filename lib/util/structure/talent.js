...
/*
With the current implementation there is stuff to do :

during composition it's possible that localProperties conflicts so resolve must also resolve localProperties
Properties.resolve does not pass the .object property to the resolvedProperties which is used later
We must also compose localProperties because we'll call talent.instance with talent.local.object so the object
must be the composition of previous talents
*/
...
const Property = util.extend({
    constructor(name, attributes) {
        ...
        if (attributes !== null && attributes !== undefined && typeof attributes !== 'object') {
            throw new TypeError('Property constructor second argument must be null/undefined or object');
        }
    ...
    set(value) {
        return this.describe(Object.assign({}, this.attributes, {value: value}));
    }
});
...
const Talent = util.extend({
    constructor(definition) {
        ...
        this.prototypeProperties = this.createPrototypeProperties();
        this.prototype = this.createPropertiesTarget(this.prototypeProperties);

        this.instanceProperties = this.createInstanceProperties();
    },

    createLocalProperties() {
        const localProperties = installProperties(this.local, LocalProperties);
        localProperties.populate(this);
        const prototypeLocalProperty = localProperties.get('prototype');
        if (prototypeLocalProperty) {
            prototypeLocalProperty.define(this);
        }
        const instanceLocalProperty = localProperties.get('instance');
    ...
    createPrototypeProperties() {
        const prototypeProperties = installProperties(this.prototype, PrototypeProperties);
        prototypeProperties.populate(this, this.local);
        return prototypeProperties;
    },

    createInstanceProperties() {
        return installProperties(this.instance, InstanceProperties);
    },

    createPropertiesTarget(properties) {
        const target = {};
        properties.link(target);
...
Talent.define({
    link(subject, ...args) {
        ...
        const linkedTalent = Object.create(this);
        // here we cannot populate only because each instance can define his own instance properties
        // it means we need a fresh instanceProperties object per call to link()
        // it can be done just be calling instancePorperties,populate on subjectProperties
        // like subjectProperies.populate = instanceProperties.populate
        // it would work but sounds like a trick
        linkedTalent.subject = subject;
        instanceProperties.populate(linkedTalent, linkedTalent.local, linkedTalent.prototype, ...args);

        subjectProperties.merge(this.prototypeProperties);
...
const Resolution = (function() {
    ...
    const PropertyResolutionStrategy = util.extend({
        from() {},
        isConflictualWithStrategyOfProperty() {
            return true;
        },
        effect() {},
        resolve() {}
    ...
    const RenameResolutionStrategy = PropertyResolutionStrategy.extend({
        ...
        isConflictualWithStrategyOfProperty(property) {
            return (
                property.resolutionStrategy.name === 'rename' &&
                property.resolutionStrategy.renameWith !== this.renameWith
            );
        },
...
Talent.define({
    compose(...talents) {
        ...
        const compositePrototypeProperties = this.prototypeProperties.compose(...talents.map(function(composedTalent) {
        ...
        compositeInstanceProperties.populate = function(linkedCompositeTalent, local, prototype, ...args) {
            this.combine(...talents.map(function(composedTalent) {
                const clone = composedTalent.instanceProperties.clone();
                clone.populate(
                    composedTalent,
                    composedTalent.local,
                    composedTalent.prototype,
                    ...args
                );
                return clone;
            }));
...
Properties.define({
    compose(...propertiesList) {
        ...
        compositeProperties.combine(...propertiesList);
        return compositeProperties;
    },

    combine(...propertiesList) {
        for (let properties of propertiesList) {
            for (let property of properties) {
                ...
                if (currentProperty) {
                    // which one is prio, property.resolutionStrategy or currentProperty.resolutionStrategy ?
                    // -> the one having ownProperty, else currentProperty
                    let firstProperty;
                    let conflictualProperty;
                    if (currentProperty.hasOwnProperty('resolutionStrategy')) {
                        firstProperty = currentProperty;
                        conflictualProperty = property;
                    } else {
                        firstProperty = property;
                        conflictualProperty = currentProperty;
                    }
                    // detect if both resolutionStrategy are conflictual
                    // true for everything except rename with different names
                    firstProperty.detectResolutionConflictWith(conflictualProperty);

                    // when resolution of firstproperty is applied we ignore resolution of conflictualProperty
                    // as it's supposed to avoid conflict
                    const resolvedProperty = firstProperty.resolutionStrategy.resolve(
                        firstProperty,
                        conflictualProperty
                    );
...
Property.define({
    detectResolutionConflictWith(otherProperty) {
        if (this.resolutionStrategy.isConflictualWithStrategyOfProperty(otherProperty.resolutionStrategy)) {
            throw new Error('conflict between resolution strategy for property named "' + this.name + '"');
        }
    }
});
...
export const test = {
    ...
    main(assert) {
        ...
        this.add('Talent.compose', function() {
            this.add('local + prototype', function() {
                ...
                assert(Object.keys(compositeTalent.prototype).length === 2);

                const subject = {};
                compositeTalent.link(subject);

                // called with a local object with right amount of properties
                assert(Object.keys(subject.firstLocal()).length === 1);
                assert(Object.keys(subject.secondLocal()).length === 2);
            });
