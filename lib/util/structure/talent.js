...
const Talent = {
    link(skill, subject, ...args) {
        ...
        function defineProperties(properties, object) {
            ...
            Object.keys(properties).forEach(function(name) {
                ...
                if (object.hasOwnProperty(name)) {
                    currentProperty = Property.create(name, Object.getOwnPropertyDescriptor(object, name));

                    // there is a conflict how do we handle it?
                    if (name in propertiesConflictResolver) {
                        const conflictResolutionStrategy = propertiesConflictResolver[name];

                        // attributes = Object.assign(attributes, {
                        //     value: composeFunction(skillValue, targetValue, 'before')
                        // });

                        if (typeof conflictResolutionStrategy === 'string') {
                            if (conflictResolutionStrategy === 'replace') {

                            } else if (conflictResolutionStrategy === 'remove') {

                            } else if (conflictResolutionStrategy === 'before') {

                            } else if (conflictResolutionStrategy === 'after') {

                            }
                        } else if (typeof conflictResolutionStrategy === 'object') {
                            if ('rename' in conflictResolutionStrategy) {

                            } else if ('around' in conflictResolutionStrategy) {

                            }
                        } else {
                            throw new Error(
                                'conflict resolution value for property ' + name + ' must be a string or an object'
                            );
                        }
                    } else {
                        throw new Error('unhandled conflict between subject & talent for property ' + name);
                    }
                } else {
                    currentProperty = Property.create(name);
                }
        ...
        const subjectPropertiesLink = defineProperties(subjectProperties, subject);
