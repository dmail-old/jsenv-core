    ...
    main() {
        ...
        this.add('Lab create a custom element matching native js object & map', function() {
            ...
            const NamedEnumerableTalent = Talent.create(function() {
                return {
                    ...
                    prototype() {
                        return {
                            get entries() {
                                return this.enumerable.entries;
                            },
                    ...
                    instance(local, prototype, subject, entries) {
                        ...
                        if (entries) {
                            for (let entry of entries) {
                                prototype.add.call(local, entry);
                            }
                        }
            ...
            const ObjectProperties = util.extend({
                ...
                seal() {
                    this.enumerable.map(function(property) {
                ...
                define(subject) {
                    this.enumerable.forEach(function(property) {
            ...
            CastableTalent.install(ObjectProperties, function(object, deep) {
                ...
                Object.keys(object).forEach(function(name) {
                    objectProperties.add(ObjectProperty.cast(name, object));
                });
                    ...
                    while (objectAncestor) {
                        Object.keys(objectAncestor).forEach(function(name) { // eslint-disable-line
                            if (objectProperties.has(name) === false) {
                                objectProperties.add(ObjectProperty.cast(name, objectAncestor));
                            }
                        });
            ...
            CastableTalent.install(ObjectProperty, function(name, object) {
                ...
                property.descriptor = Object.getOwnPropertyDescriptor(object, name);
                return property;
            });
