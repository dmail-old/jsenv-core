...
export const test = {
    ...
    main() {
        ...
        this.add('Lab create a custom element matching native js object & map', function() {
            ...
            // maintenant il me faut la possibilité de caster, ou scanner, fin bref instancier depuis quelque chose
            // une structure
            // pour cela j'ai besoin d'une manière d'analyser l'argument
            // et en conséquence de faire quelque chose sur l'object
            // une sorte d'initiliazer en fait tel qu'on peut le trouver sur Array ou Map

            const ObjectElement = Composite.extend();

            // objectProperties est à la fois un component et un composite
            // bon pour le moment partons du principe que c'est simple à faire
            const ObjectProperties = ObjectElement.compose();
            const ObjectProperty = ObjectProperties.compose();
            const ObjectPropertiesFiller = ObjectProperties.compose('filler');

            const NamedEnumeratorComponent = Component.create('namedEnumerator');
            NamedEnumeratorComponent.refine({
                installed() {
                    this.entries = {};
                },
            ...
            ObjectProperties.compose(NamedEnumeratorComponent);
            // this means that objectProperties doesn't have any methods on it's own, only a namedEnumerator property
            // which is holding the method & state
            // this is clean sometimes but here it feels like there is too much encapsulation
            // so sometimes we want component to remain separate, meaning it behaves like a component
            // and sometimes it's ok to compose methods on the same object because it makes sense
            ...
            ObjectPropertiesFiller.method({
                fill(objectProperties, object, deep) {
            ...
            const MapElement = Composite.extend();
            const MapEntries = MapElement.compose();
            const MapEntry = MapEntries.compose();
            const MapEntriesFiller = MapEntries.compose('filler');
            const MapEntryFiller = MapEntry.compose('filler');
            ...
            MapEntryFiller.method({
                fill(mapEntry, name, value) {
                    mapEntry.name = name;
