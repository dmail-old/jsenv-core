...
const Linker = util.extend({
    ...
    initialize(element) {
        ...
        this.initiliazer(element.value);
    },
    ...
    remove(element) {
        const linker = this;
        const index = linker.indexOf(element);
        Array.prototype.splice(linker, index, 1);
...
const Link = util.extend({
    constructor() {
        // link may concern more then one element, even if 99% of the time it's only one
        this.fragment = [];
    },
...
Element.define({
    ...
    addChild(firstArg) {
        if (this.hasOwnProperty('linker') === false) {
            this.linker = Linker.create();
            this.linker.initialize(this);
        }
