            ...
            const Navigator = TreeNavigator.compile(function() {
                return {
                    parent(node) {
                        ...
                        // here Object.getPrototypeOf(node.navigator) === this
                        return parent;
                    },
...
function createInstalledComponent(component, ...args) {
    const installedComponent = Object.create(component);
    installedComponent.installed(...args);
    return installedComponent;
}

function createExposedComponent(installedComponent) {
    const exposedComponent = Object.create(installedComponent);
    installedComponent.methods.expose(exposedComponent);
    return exposedComponent;
}

function getInstalledComponentFromExposedComponent(exposedComponent) {
    return Object.getPrototypeOf(exposedComponent);
}

function setExposedComponentOnComposite(composite, exposedComponent) {
    Object.defineProperty(composite, exposedComponent.name, {
        enumerable: false,
        configurable: false,
        writable: false,
...
function getExposedComponentOnComposite(composite, component) {
    const name = component.name;
    let exposedComponent;

    if (name in composite) {
        exposedComponent = composite[name];
    } else {
        exposedComponent = null;
    }
...
const ComponentMethod = util.extend({
    constructor(component, name, fn) {
        ...
        // const defaultComponent = component.compiled ? Object.getPrototypeOf(component) : component;

        // prepare a delegated version
        this.delegated = function(composite, ...args) {
            let owner;
            const exposedComponent = getExposedComponentOnComposite(composite, component);
            let methods;

            if (exposedComponent) {
                // here owner is the composite exposedComponent
                // the delegation concept happens here :
                // -> callings component's B method is delegated to component's A method
                let exposedComponentInstalled = getInstalledComponentFromExposedComponent(exposedComponent);
                owner = exposedComponentInstalled;
                methods = exposedComponentInstalled.methods;
                ...
                let installedComponent = this;
                owner = installedComponent;
                methods = installedComponent.methods;
            } else {
                // here, because we don't have exposed component AND the component is not inheritable
                // we got two option : throw or call the default method
                // right now we call the default method
                // if the component is compiled we must get the default method
                owner = component;
        ...
        this.exposed = function(...args) {
            ...
            const installedComponent = getInstalledComponentFromExposedComponent(exposedComponent);
...
const ComponentMethods = util.extend({
    ...
    add(method) {
        ...
        // for convenience delegated method are installed on the component
...
const Component = util.extend({
    ...
    compile(methods) {
        ...
        const compiledMethods = ComponentMethods.create(compiledComponent, Object.create(this.methods.map));
    ...
    installed() {
        // do something when the component is installed on a composite
    },

    install(composite, ...args) {
        const component = this;
        component.composite = composite;

        const installedComponent = createInstalledComponent(component, ...args);
        const exposedComponent = createExposedComponent(installedComponent);
        setExposedComponentOnComposite(composite, exposedComponent);
...
TreeNavigator.inheritable = true;
