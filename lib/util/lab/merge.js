...
const CopyTransformation = Transformation.extend({
    ...
    refine(copyOfElement, element) {
        for (let constituent of element) {
            const constituentTransformation = CopyTransformation.create(constituent, copyOfElement);
            const importedConstituent = constituentTransformation.prepare();
            copyOfElement.appendChild(importedConstituent);
...
ObjectPropertyElement.refine({
    copy() {
        const copy = Element.copy.call(this);
...
const CombinePropertyReaction = CombineObjectReaction.extend({
    ...
    handleConstituant(compositeProperty, constituantName, firstComponent, secondComponent) {
        ...
        if (firstConstituant) {
            ...
            // console.log('reaction product ?', reactionProduct);
            reaction.proceed();
        }
    }
});
