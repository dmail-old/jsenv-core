...
const PrototypeSet = proto.extend('PrototypeSet', {
    prototype: null,
    set: [],

    constructor(...args) {
        this.set = this.set.slice();
        this.set.push(...args);
    },

    add(prototype) {
        this.set.push(prototype);
    },

    // instead of this I would prefer adding a detach(), unregister() or sthing like that
    // on the object itself once he is added to the PrototypeSet
    remove(prototype) {
        var set = this.set;
        set.splice(set.indexOf(prototype), 1);
    },
    ...
    findByName(name) {
        let foundPrototype = this.set.find(function(prototype) {
    ...
    removeByName(name) {
        return this.remove(this.findByName(name));
    }
});
...
const TransformerPrototypeSet = PrototypeSet.extend('TransformerPrototypeSet', {
...
const Transformation = CompilableNode.extend({
    ...
    transformerPrototypeSet: TransformerPrototypeSet.create(Transformer),

    registerTransformer(name, transformMethod) {
        return this.transformerPrototypeSet.register({name: name, transformMethod: transformMethod});
    },

    unregisterTransformer(nameOrPrototype) {
        if (typeof nameOrPrototype === 'string') {
            return this.transformerPrototypeSet.removeByName(nameOrPrototype);
        }
        return this.transformerPrototypeSet.remove(nameOrPrototype);
    },
    ...
    populate(syntaxNode) {
        ...
        this.transformer = this.transformerPrototypeSet.generateByName(syntaxNode.value, ...args);
    },
...
export {CompilableNode, Tokenizer, Parser, PrototypeSet};
export {Parameter, Transformation, Expression, Variable, Constant, Template};
