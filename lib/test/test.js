...
var Test = proto.extend('Test', {
    ...
    */
    ...
    emit() {
        var reporter = this.getSelfOrParentProperty('reporter');
        ...
        if (this.skipReason) {
            ...
            if (this.hasOwnProperty('agents')) {
                canRunOnAgent = this.agents.some(function(agent) {
                    return global.engine.agent.is(agent);
                });
            } else {
                canRunOnAgent = true;
            }
            ...
            if (canRunOnAgent === false) {
                this.skip('unsupported agent : ' + global.engine.agent);
            } else if (global.engine.agent.name !== 'node' && this.hasNodeModuleImport()) {
                this.skip('unsupported agent : ' + global.engine.agent);
            }
        }
    },
    ...
    createStatePromise() {
        ...
        var explicitStatePromise = new Promise(function(resolve, reject) {
            this.resolve = resolve;
            this.reject = reject;
        }.bind(this));
        var callStatePromise = this.createCallPromise();

        this.promise = callStatePromise;

        return Promise.race([callStatePromise, explicitStatePromise]);
    },
