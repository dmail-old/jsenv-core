...
import moduleScriptNames from './module-script-name.js';
...
function fetchSourceMapData(source, rootURL) {
    ...
    if (sourceMapURL) {
        ...
        if (base64SourceMapRegexp.test(sourceMapURL)) {
            ...
            sourceMapURL = new URL(sourceMapURL, rootURL);
            if (sourceMapURL.indexOf('file:///') === 0) {
                sourceMapURL = sourceMapURL.slice('file:///'.length);
            }
            ...
            if (sourcesContent) {
                return Promise.all(sourceMap.sources.map(function(source, i) {
                    ...
                    if (content) {
                        ...
                        if (sourceMapUrl) {
                            sourceMapLocation = new URL(source, sourceMapUrl).href;
                        } else {
                            sourceMapLocation = new URL(source, jsenv.baseURL);
                        }
