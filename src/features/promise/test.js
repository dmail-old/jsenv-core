...
import {expect, presence, listenOnce, fail} from '/test-helpers.js';

const test = expect({
    'presence': presence('Promise'),
    ...
    'rejection-handled'(Promise) {
        ...
        const reason = 'foo';
        const promise = Promise.reject(reason);
        const waitEventDuration = 80; // engine has 100ms to trigger the event
        return new Promise(function(resolve) {
            setTimeout(
                function() {
